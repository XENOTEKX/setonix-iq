#!/bin/bash
# run_mf_iso_dna_100k_2node_full.sh — IQ-TREE3 MF-iso full run (MF+SPR), 2-node MPI.
#
# DNA, 100K sites (100 taxa), 2 MPI ranks × 103 OpenMP threads = 206T total.
...
#   across 2 MPI ranks; SPR tree search runs within each rank's OMP pool after MF.
#
#   Acceptance gate:
#     - lnL = -5,692,984.539 ± 0.1  (SPR ref: 168425674 / 168580367)
#     - best model = F81+F+G4
#     - MF wall < baseline MF wall (FCA speedup confirmed)
#     - Phase 0.5 broadcast fires (filterRatesMPI diag line present)
#
# EXPECTED_LNL: -5692984.539 (from run_baseline_dna_100k_spr.sh / 168580367).
#
# Binary:  /scratch/dx61/as1708/iqtree3-mf-iso/build-mpi-iso/iqtree3-mpi

#PBS -N mf-iso-dna-100k-2n-full
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=208
#PBS -l mem=1020GB
...
LABEL="DNA_100k_mfiso_np2_full_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  DNA 100K MF-iso Full Run — 2-node MPI, MF+SPR (-m TEST)"
echo "║  run_id:       ${RUN_ID}"
...
grep -E "filterRatesMPI fired" "${WORK_DIR}/mf_diag.log" 2>/dev/null | sed 's/^/    /' || echo "    (not fired — expected only at np>=2)"
...
# Full-run SPR lnL should match SPR-tree reference within 0.1.
...
if rep_ll is not None and EXPECTED_LNL is not None:
    ...
    verify.append({"file": os.path.basename(alignment), "status": "pass" if diff < 0.1 else "fail",
                   ...
                   "note": "Full-run SPR lnL vs SPR baseline ref (168425674); tol=0.1"})
...
record = {
    ...
    "timing": [{"command": f"mpirun -np {nranks} -rf rankfile --output-filename rank_logs numactl --localalloc iqtree3-mpi -s alignment_100000.phy -m TEST -T {omp_per_rank} -seed ${SEED}",
    ...
    "summary": {
        ...
        "mf_wall_s": mf_wall,
        "lnL": rep_ll, "best_model": best_model, "all_pass": iqrc == 0,
    },
    ...
    "profile": {"nranks": nranks, "omp_per_rank": omp_per_rank, "placement": "mpi_2node_full_perrank_logs"},
...
print(f"[2node] lnL={rep_ll}  best_model={best_model}  broadcast_fired={broadcast_fired}  local_pruned={local_pruned}")
PYEOF
