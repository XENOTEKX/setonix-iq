#!/bin/bash
# run_mf_iso_dna_100k_4node_full.sh — IQ-TREE3 MF-iso full run (MF+SPR), 4-node MPI.
#
# DNA, 100K sites (100 taxa), 4 MPI ranks × 103 OpenMP threads = 412T total.
# 4 × Sapphire Rapids exclusive nodes (normalsr, 104 cores each).
#
# PURPOSE — T4 from §5.1 test matrix: direct measurement of FCA ModelFinder
#   (Phase 0.5+0.6) + SPR tree search at np=4 on the DNA 100K dataset.
#   Closes the gap that was previously extrapolated from np=2 data.
#
#   Acceptance gate:
#     - lnL = -5,692,984.539 ± 0.1  (SPR ref: 168425674 / 168580367)
#     - best model = F81+F+G4
#     - MF wall < DNA 100K np=2 full MF wall (scaling benefit confirmed)
...
# np=2 full ref (168584737): lnL -5692984.539, F81+F+G4 (2×normalsr)
# SPR ref (168425674): lnL -5692984.539, F81+F+G4 (normalsr 1×103T)

#PBS -N mf-iso-dna-100k-4n-full
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=416
#PBS -l mem=2040GB
...
LABEL="DNA_100k_mfiso_np4_full_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  DNA 100K MF-iso Full Run — 4-node MPI, MF+SPR (-m TEST)"
echo "║  run_id:       ${RUN_ID}"
...
echo "║  SPR ref (168425674): lnL -5692984.539, F81+F+G4 (normalsr 103T)"
echo "╚══════════════════════════════════════════════════════════════╝"
...
echo "[4node] mpirun --output-filename ${RANK_LOGS_DIR}/"
...
echo "[4node] BINDINGS:           $(wc -l < "${WORK_DIR}/rank_bindings.log" 2>/dev/null || echo 0) lines"
