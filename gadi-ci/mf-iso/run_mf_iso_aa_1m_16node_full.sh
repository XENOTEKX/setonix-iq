#!/bin/bash
# run_mf_iso_aa_1m_16node_full.sh — IQ-TREE3 MF-iso full run (MF+SPR), 16-node MPI.
#
...
#
# PURPOSE — End-to-end parity validation at np=16: FCA ModelFinder (Phase 0.5+0.6)
#   + SPR tree search across 16 nodes.  Upper end of the full-run Thread Scaling
#   chart; demonstrates near-linear MF speedup at scale.
#
#   Acceptance gate:
#     - lnL = -78605196.573 ± 1.0  (SPR ref: 168425491, normalsr 103T, LG+G4)
#     - best model = LG+G4
#     - filterRatesMPI fires (Phase 0.5 broadcast confirmed)
#     - MF wall < np=8 full run (168586094): ~1443 s (scaling benefit confirmed)
#
# Binary:  /scratch/dx61/as1708/iqtree3-mf-iso/build-mpi-iso/iqtree3-mpi
# Build tag:    mf_iso_phase0.5_0.6_icx_avx512_mftime
# np=8 full ref (168586094): lnL -78605196.506, LG+G4, MF 1443.892 s (8×normalsr)
# SPR ref (168425491): lnL -78605196.573, LG+G4, MF 7587.459 s (normalsr 1×103T)

#PBS -N mf-iso-aa-1m-16n-full
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=1664
#PBS -l mem=8160GB
...
LABEL="AA_1m_mfiso_np16_full_seed${SEED}"
...
echo "║  np=8 full ref (168586094): lnL -78605196.506, LG+G4, MF 1443.892 s"
echo "╚══════════════════════════════════════════════════════════════╝"
