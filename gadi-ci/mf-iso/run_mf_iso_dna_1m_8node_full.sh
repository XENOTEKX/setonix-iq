#!/bin/bash
# run_mf_iso_dna_1m_8node_full.sh — IQ-TREE3 MF-iso full run (MF+SPR), 8-node MPI.
#
# DNA, 1M sites (100 taxa), 8 MPI ranks × 103 OpenMP threads = 824T total.
...
#
#   Acceptance gate:
#     - lnL = -59208019.212 ± 0.5  (SPR ref: 168425675, normalsr 103T, F81+F+G4)
#     - best model = F81+F+G4
#     - filterRatesMPI fires (Phase 0.5 broadcast confirmed)
#     - MF wall < np=2 MF wall (scaling benefit vs np=2 confirmed)
#
# Binary:  /scratch/dx61/as1708/iqtree3-mf-iso/build-mpi-iso/iqtree3-mpi
# Build tag:    mf_iso_phase0.5_0.6_icx_avx512_mftime
# SPR ref (168425675): lnL -59208019.212, F81+F+G4, wall 3500.825 s (normalsr 1×103T)
# CLX ref (168422813): lnL -59208019.212, F81+F+G4, wall 6114.450 s (normal CLX 47T)

#PBS -N mf-iso-dna-1m-8n-full
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=832
#PBS -l mem=4080GB
...
LABEL="DNA_1m_mfiso_np8_full_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  DNA 1M MF-iso Full Run — 8-node MPI, MF+SPR (-m TEST)"
echo "║  run_id:       ${RUN_ID}"
...
echo "║  SPR ref (168425675): lnL -59208019.212, F81+F+G4, wall 3500.825 s (normalsr 103T)"
echo "╚══════════════════════════════════════════════════════════════╝"
...
if rep_ll is not None:
    ...
    verify.append({"file": os.path.basename(alignment), "status": "pass" if diff < TOL else "fail",
                   ...
                   "note": "Full-run SPR lnL vs SPR baseline ref (168425675); tol=0.5"})
