#!/bin/bash
# run_mf_iso_aa_100k_4node_full.sh — IQ-TREE3 MF-iso full run (MF+SPR), 4-node MPI.
#
...
#
# PURPOSE — T3 from §5.1 test matrix: direct measurement of FCA ModelFinder
#   (Phase 0.5+0.6) + SPR tree search at np=4.  Closes the gap that was previously
#   inferred from AA 1M scaling ratios.
#
#   Acceptance gate:
#     - lnL = -7,541,976.860 ± 0.1  (SPR ref: 168425673 / 168573852)
#     - best model = LG+G4
#     - MF wall < AA 100K np=2 full MF wall (scaling benefit confirmed)
...
# np=2 full ref (168584736): lnL -7541976.860, LG+G4, MF 150.567 s (2×normalsr)
# SPR ref (168425673): lnL -7541976.860, LG+G4 (normalsr 1×103T)

#PBS -N mf-iso-aa-100k-4n-full
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=416
#PBS -l mem=2040GB
...
LABEL="AA_100k_mfiso_np4_full_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  AA 100K MF-iso Full Run — 4-node MPI, MF+SPR (-m TEST)"
echo "║  run_id:       ${RUN_ID}"
...
echo "║  SPR ref (168425673): lnL -7541976.860, LG+G4 (normalsr 103T)"
echo "╚══════════════════════════════════════════════════════════════╝"
