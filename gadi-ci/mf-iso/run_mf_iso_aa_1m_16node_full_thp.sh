#!/bin/bash
# run_mf_iso_aa_1m_16node_full_thp.sh — IQ-TREE3 MF-iso full run (MF+SPR), 16-node MPI, THP binary.
#
...
#
# PURPOSE — Measure THP (madvise MADV_HUGEPAGE on central_partial_lh) gain at np=16.
#   Binary: commit c8f11a24 (test_MF2), md5 a103bc6c97860145033206c47b184367.
#   Patch:  setonix-iq/patches/iqtree3/0004-thp-partial-lh-madvise.patch
#   vs pre-THP np=16 run 168635616: MF 1122.363 s, IPC 1.337, LLC miss 85.27%
#
#   Acceptance gate:
#     - lnL = -78605196.573 ± 1.0  (SPR ref: 168425491, normalsr 103T, LG+G4)
#     - best model = LG+G4
#     - filterRatesMPI fires (Phase 0.5 broadcast confirmed)
#     - MF wall < pre-THP np=16 run (168635616): 1122.363 s
#     - IPC improvement vs 168635616 (1.337) expected if THP reduces TLB stalls
#
# Binary:  /scratch/dx61/as1708/iqtree3-mf-iso/build-mpi-iso/iqtree3-mpi
# Build tag:    mf_iso_phase0.5_0.6_thp_icx_avx512_mftime
# pre-THP np=16 ref (168635616): lnL -78605196.497, LG+G4, MF 1122.363 s, IPC 1.337
# SPR ref (168425491): lnL -78605196.573, LG+G4, MF 7587.459 s (normalsr 1×103T)

#PBS -N mf-iso-aa-1m-16n-full-thp
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=1664
#PBS -l mem=8160GB
...
LABEL="AA_1m_mfiso_np16_full_thp_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  AA 1M MF-iso Full Run — 16-node MPI, MF+SPR (-m TEST)"
echo "║  run_id:       ${RUN_ID}"
...
echo "║  node I:       ${HOST_I}  (rank 8)"
echo "║  node J:       ${HOST_J}  (rank 9)"
echo "║  node K:       ${HOST_K}  (rank 10)"
echo "║  node L:       ${HOST_L}  (rank 11)"
echo "║  node M:       ${HOST_M}  (rank 12)"
echo "║  node N:       ${HOST_N}  (rank 13)"
echo "║  node O:       ${HOST_O}  (rank 14)"
echo "║  node P:       ${HOST_P}  (rank 15)"
echo "║  binary:       $(basename "${IQTREE}")"
echo "║  alignment:    $(basename "${ALIGNMENT}")"
echo "║  work_dir:     ${WORK_DIR}"
echo "║  rank logs:    ${RANK_LOGS_DIR}/"
echo "║  branch:       test_MF2 (c8f11a24) + THP patch 0004"
echo "║  pre-THP np=16 ref (168635616): MF 1122.363 s, IPC 1.337, LLC miss 85.27%"
echo "╚══════════════════════════════════════════════════════════════╝"
...
echo "[16node] mpirun --output-filename ${RANK_LOGS_DIR}/"
...
echo "[16node] BINDINGS:           $(wc -l < "${WORK_DIR}/rank_bindings.log" 2>/dev/null || echo 0) lines"
...
record = {
    ...
    "build_tag":     "mf_iso_phase0.5_0.6_thp_icx_avx512_mftime",
    "branch":        "test_MF2",
    "source_commit": "c8f11a24",
    "non_canonical": True,
    "non_canonical_label": "MF-iso Phase 0.5+0.6+THP (MADV_HUGEPAGE on central_partial_lh) · ICX+MPI · AVX-512",
    "group":         "mf_iso_scaling",
    "thp_patch":     "0004-thp-partial-lh-madvise.patch",
}
