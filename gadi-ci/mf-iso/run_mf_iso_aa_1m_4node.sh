#!/bin/bash
# run_mf_iso_aa_1m_4node.sh — IQ-TREE3 MF-iso ModelFinder benchmark, 4-node MPI.
#
# AA, 1M sites (100 taxa), 4 MPI ranks × 103 OpenMP threads = 412T total.
# 4 × Sapphire Rapids exclusive nodes (normalsr, 104 cores each).
#
# PURPOSE — Phase 0.5+0.6 validation at np=4 for AA 1M.
#   Acceptance gates:
#     - lnL matches np=2 result (within 0.01)
#     - best model matches reference (LG+G4)
#     - filterRatesMPI fires at np=4 (Phase 0.5 broadcast confirmed)
#     - MF wall < np=2 wall (scaling benefit confirmed)
#     - |lnL − (−78605196.573)| < 1.0  (SPR cross-platform parity)
#
# Binary:  /scratch/dx61/as1708/iqtree3-mf-iso/build-mpi-iso/iqtree3-mpi
# SPR reference (168425491): lnL -78605196.573, LG+G4, MF 7587.459 s (normalsr 103T)
# CLX reference (168425490): lnL -78605196.573, LG+G4, MF 16308.318 s (normal CLX 47T)

#PBS -N mf-iso-aa-1m-4n
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=416
#PBS -l mem=2040GB
...
LABEL="AA_1m_mfiso_np4_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  AA 1M MF-iso Benchmark — 4-node MPI, ModelFinder-only (-m TESTONLY)"
echo "║  run_id:       ${RUN_ID}"
...
echo "║  CLX ref (168425490): lnL -78605196.573, LG+G4, MF 16308.318 s (normal CLX 47T)"
echo "╚══════════════════════════════════════════════════════════════╝"
...
echo "[4node] ModelFinder-only run, ${NRANKS} ranks × ${OMP_PER_RANK} OMP across 4 nodes"
START_EPOCH=$(date +%s)
...
for f in "${RANK_LOGS_DIR}"/*/rank.*/stdout; do
    ...
    echo "  rank ${rank}: $(wc -l < "$f") lines"
done

{ for r in "${WORK_DIR}"/rank_*.stdout.log; do
    [[ -f "$r" ]] && grep -E '^MF-TIME: ' "$r" || true; done
} > "${WORK_DIR}/mf_time.log" 2>/dev/null || true
{ for r in "${WORK_DIR}"/rank_*.stdout.log; do
    [[ -f "$r" ]] && grep -E '^MF-MPI-DIAG: ' "$r" || true; done
} > "${WORK_DIR}/mf_diag.log" 2>/dev/null || true
...
echo "[4node] Phase 0.5 broadcast:"
grep -E "filterRatesMPI fired" "${WORK_DIR}/mf_diag.log" 2>/dev/null | sed 's/^/    /' || echo "    (not yet fired — expected at np>=2)"
...
broadcast_fired = False; bcast_ok_rates_size = None; local_pruned = None
if os.path.isfile(diag_log):
    for line in open(diag_log, errors="replace"):
        if "filterRatesMPI fired at" in line:
            ...
            m = re.search(r"local_pruned=(\d+)", line)
            if m: local_pruned = int(m.group(1))

EXPECTED_LNL_SPR = -78605196.573  # 168425491 (normalsr 103T full run)

verify = []
if rep_ll is not None:
    diff = abs(rep_ll - EXPECTED_LNL_SPR)
    verify.append({"file": os.path.basename(alignment), "status": "pass" if diff < 1.0 else "fail",
                   "expected_spr": EXPECTED_LNL_SPR, "reported": rep_ll, "diff": round(diff, 6)})
...
record = {
    ...
    "timing": [{"command": f"mpirun -np {nranks} -rf rankfile --output-filename rank_logs numactl --localalloc iqtree3-mpi -s alignment_1000000.phy -m TESTONLY -T {omp_per_rank} -seed ${SEED}",
    ...
    "phase0_5_broadcast": {"fired": broadcast_fired, "ok_rates_size": bcast_ok_rates_size, "local_pruned": local_pruned},
    "reference_run_spr": "168425491", "reference_lnL_spr": -78605196.573,
    "reference_best_model_spr": "LG+G4", "reference_mf_wall_spr_s": 7587.459,
    "reference_run_clx": "168425490", "reference_lnL_clx": -78605196.573,
    "reference_best_model_clx": "LG+G4", "reference_mf_wall_clx_s": 16308.318,
    "env": {
    ...
    "profile": {"nranks": nranks, "omp_per_rank": omp_per_rank, "placement": "mpi_4node_testonly_perrank_logs"},
...
print(f"[4node] lnL={rep_ll}  best_model={best_model}  MF_wall={mf_wall_direct}s  broadcast_fired={broadcast_fired}  local_pruned={local_pruned}")
print(f"[4node] SPR ref: lnL=-78605196.573  LG+G4  MF=7587.459s (168425491)")
PYEOF
