#!/bin/bash
# run_mf_iso_dna_100k_2node.sh — IQ-TREE3 MF-iso ModelFinder benchmark, 2-node MPI.
#
...
#
# PURPOSE — Phase 0.5+0.6 validation at np=2 for DNA 100K.
#   Acceptance gates:
#     - lnL matches run_mf_iso_dna_100k_1node result (within 0.01)
#     - best model matches np=1
#     - filterRatesMPI fires at np=2 (Phase 0.5 broadcast confirmed)
#     - MF wall < np=1 wall (scaling confirmed)
#
# EXPECTED_LNL: TBD — update after run_baseline_dna_100k_spr.sh completes.
#
# Binary:  /scratch/dx61/as1708/iqtree3-mf-iso/build-mpi-iso/iqtree3-mpi

#PBS -N mf-iso-dna-100k-2n
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=208
#PBS -l mem=1020GB
...
LABEL="DNA_100k_mfiso_np2_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  DNA 100K MF-iso Benchmark — 2-node MPI, ModelFinder-only (-m TESTONLY)"
echo "║  run_id:       ${RUN_ID}"
...
record = {
    ...
    "timing": [{"command": f"mpirun -np {nranks} -rf rankfile --output-filename rank_logs numactl --localalloc iqtree3-mpi -s alignment_100000.phy -m TESTONLY -T {omp_per_rank} -seed ${SEED}",
