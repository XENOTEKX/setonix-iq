#!/bin/bash
# run_mf_iso_xlarge_8node_full.sh — IQ-TREE3 MF-iso full run (MF+SPR), 8-node MPI.
#
# DNA, xlarge_mf.fa (200 taxa × 100,000 sites, 98,858 distinct patterns).
# 8 MPI ranks × 103 OpenMP threads = 824T total.
# 8 × Sapphire Rapids exclusive nodes (normalsr, 104 cores each).
#
# PURPOSE — Parity benchmark at scale: FCA ModelFinder (Phase 0.5+0.6)
#   + SPR tree search on xlarge DNA dataset.  Demonstrates FCA speedup over
#   MF2 Full at 8 nodes; target is to beat MF2 Full np=8 ref time of 139.483s.
#
#   Acceptance gate:
#     - lnL = -10956936.089 ± 1.0  (MF2 Full ref: job 168195261, SYM+G4)
#     - best model = SYM+G4
#     - filterRatesMPI fires (Phase 0.5 broadcast confirmed)
#     - total wall < 139.483s (MF2 Full np=8 job 168195261, ref lnL -10956936.145)
#
# Binary:  /scratch/dx61/as1708/iqtree3-mf-iso/build-mpi-iso/iqtree3-mpi
# Build tag:    mf_iso_phase0.5_0.6_icx_avx512_mftime
# MF2 Full 8-node ref (168195261): lnL -10956936.145, SYM+G4, wall 139.483s (8×normalsr)
# MF2 Full 1-node ref (np=1): lnL -10956936.089, SYM+G4, wall 493.972s (normalsr 1×104T)

#PBS -N mf-iso-xlarge-8n-full
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=832
#PBS -l mem=3200GB
...
ALIGNMENT="${ALIGNMENT:-/scratch/rc29/as1708/iqtree3-3.1.2/benchmarks/xlarge_mf.fa}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  xlarge_mf MF-iso Full Run — 8-node MPI, MF+SPR (-m TEST)"
echo "║  run_id:       ${RUN_ID}"
...
echo "║  MF2 Full 8-node ref (168195261): lnL -10956936.145, SYM+G4, wall 139.483s"
echo "╚══════════════════════════════════════════════════════════════╝"
...
if rep_ll is not None:
    ...
    verify.append({"file": os.path.basename(alignment), "status": "pass" if diff < TOL else "fail",
                   ...
                   "note": "Full-run SPR lnL vs MF2 Full 8-node ref (168195261, lnL -10956936.145); tol=1.0"})

record = {
    "run_id": rid, "label": label,
    "platform": "gadi", "run_type": "mf_iso",
    "dataset": alignment, "dataset_short": "${DATASET_SHORT}",
    "data_type": "${DATA_TYPE}", "seq_len": 100000, "n_taxa": 200,
    "threads": threads, "seed": ${SEED},
    "model_finder_only": False,
    "timing": [{
        "command": f"mpirun -np {nranks} -rf rankfile --output-filename rank_logs numactl --localalloc iqtree3-mpi -s xlarge_mf.fa -m TEST -T {omp_per_rank} -seed ${SEED}",
