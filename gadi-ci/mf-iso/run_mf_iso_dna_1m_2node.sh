#!/bin/bash
# run_mf_iso_dna_1m_2node.sh — IQ-TREE3 MF-iso ModelFinder benchmark, 2-node MPI.
#
# DNA, 1M sites (100 taxa), 2 MPI ranks × 103 OpenMP threads = 206T total.
# 2 × Sapphire Rapids exclusive nodes (normalsr, 104 cores each).
#
# PURPOSE — Phase 0.5+0.6 validation at np=2 for DNA 1M.
#   Acceptance gates:
#     - lnL matches np=1 result (within 0.01)
#     - best model matches np=1 and CLX reference (F81+F+G4)
#     - filterRatesMPI fires at np=2 (Phase 0.5 broadcast confirmed)
#     - MF wall < np=1 wall (scaling benefit confirmed)
#     - |lnL − (−59208019.212)| < 0.5  (CLX SPR cross-platform parity)
#
# Binary:  /scratch/dx61/as1708/iqtree3-mf-iso/build-mpi-iso/iqtree3-mpi
#          CLX reference (168422813): lnL -59208019.212, F81+F+G4, MF 10230 s

#PBS -N mf-iso-dna-1m-2n
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=208
#PBS -l mem=1020GB
...
LABEL="DNA_1m_mfiso_np2_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  DNA 1M MF-iso Benchmark — 2-node MPI, ModelFinder-only (-m TESTONLY)"
echo "║  run_id:       ${RUN_ID}"
...
EXPECTED_LNL_CLX = -59208019.212  # 168422813

verify = []
if rep_ll is not None:
    diff = abs(rep_ll - EXPECTED_LNL_CLX)
    verify.append({"file": os.path.basename(alignment), "status": "pass" if diff < 0.5 else "fail",
                   "expected_clx": EXPECTED_LNL_CLX, "reported": rep_ll, "diff": round(diff, 6)})
...
print(f"[2node] CLX ref: lnL=-59208019.212  F81+F+G4  MF=10230s")
PYEOF
