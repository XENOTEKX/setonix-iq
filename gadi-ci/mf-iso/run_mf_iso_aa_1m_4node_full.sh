#!/bin/bash
# run_mf_iso_aa_1m_4node_full.sh — IQ-TREE3 MF-iso full run (MF+SPR), 4-node MPI.
#
# AA, 1M sites (100 taxa), 4 MPI ranks × 103 OpenMP threads = 412T total.
# 4 × Sapphire Rapids exclusive nodes (normalsr, 104 cores each).
#
# PURPOSE — End-to-end parity validation at np=4: FCA ModelFinder (Phase 0.5+0.6)
#   + SPR tree search across 4 nodes.  Central scaling datapoint for the Thread
#   Scaling chart.
#
#   Acceptance gate:
...
LABEL="AA_1m_mfiso_np4_full_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  AA 1M MF-iso Full Run — 4-node MPI, MF+SPR (-m TEST)"
echo "║  run_id:       ${RUN_ID}"
