#!/bin/bash
# run_mf_iso_aa_1m_2node.sh — IQ-TREE3 MF-iso ModelFinder benchmark, 2-node MPI.
#
# AA, 1M sites (100 taxa), 2 MPI ranks × 103 OpenMP threads = 206T total.
# 2 × Sapphire Rapids exclusive nodes (normalsr, 104 cores each).
#
# PURPOSE — Phase 0.5+0.6 validation at np=2 for AA 1M.
#   Acceptance gates:
#     - lnL matches np=2 result (within 0.01 vs np=4)
#     - best model matches reference (LG+G4)
#     - filterRatesMPI fires at np=2 (Phase 0.5 broadcast confirmed)
#     - MF wall < SPR baseline (7587.459 s from 168425491)
...
LABEL="AA_1m_mfiso_np2_seed${SEED}"
...
if [[ "${#HOSTS[@]}" -ne 2 ]]; then
    echo "ERROR: expected 2 nodes, got ${#HOSTS[@]}" >&2; exit 9
fi
HOST_A="${HOSTS[0]}"
HOST_B="${HOSTS[1]}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  AA 1M MF-iso Benchmark — 2-node MPI, ModelFinder-only (-m TESTONLY)"
echo "║  run_id:       ${RUN_ID}"
echo "║  ranks × OMP: ${NRANKS} × ${OMP_PER_RANK}  (= ${TOTAL_THREADS}T)"
echo "║  node A:       ${HOST_A}  (rank 0 — owns ref-subst family)"
echo "║  node B:       ${HOST_B}  (rank 1 — owns non-ref family)"
...
echo "[2node] Phase 0.5 broadcast:"
...
record = {
    ...
    "profile": {"nranks": nranks, "omp_per_rank": omp_per_rank, "placement": "mpi_2node_testonly_perrank_logs"},
...
print(f"[2node] lnL={rep_ll}  best_model={best_model}  MF_wall={mf_wall_direct}s  broadcast_fired={broadcast_fired}  local_pruned={local_pruned}")
print(f"[2node] SPR ref: lnL=-78605196.573  LG+G4  MF=7587.459s (168425491)")
PYEOF
