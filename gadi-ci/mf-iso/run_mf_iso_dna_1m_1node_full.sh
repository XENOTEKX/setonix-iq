#!/bin/bash
# run_mf_iso_dna_1m_1node_full.sh — IQ-TREE3 MF-iso full run (MF+SPR), 1-node MPI.
#
# DNA, 1M sites (100 taxa), 1 MPI rank × 103 OpenMP threads = 103T total.
...
#   non-MPI SPR baseline (168425675).
#
#   Acceptance gate:
#     - lnL = -59208019.212 ± 1.0  (SPR ref: 168425675, normalsr 103T, F81+F+G4)
#     - best model = F81+F+G4
...
# SPR ref (168425675): lnL -59208019.212, F81+F+G4, MF 3500.825 s (normalsr 1×103T)
# CLX ref (168422813): lnL -59208019.212, F81+F+G4, MF 10230 s (normal CLX 47T)

#PBS -N mf-iso-dna-1m-1n-full
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=104
#PBS -l mem=510GB
...
LABEL="DNA_1m_mfiso_np1_full_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  DNA 1M MF-iso Full Run — 1-node MPI, MF+SPR (-m TEST)"
echo "║  run_id:       ${RUN_ID}"
...
echo "║  SPR ref (168425675): lnL -59208019.212, F81+F+G4, MF 3500.825 s (normalsr 103T)"
echo "╚══════════════════════════════════════════════════════════════╝"
...
if rep_ll is not None:
    ...
    verify.append({"file": os.path.basename(alignment), "status": "pass" if diff < TOL else "fail",
                   ...
                   "note": "Full-run SPR lnL vs SPR baseline ref (168425675); tol=1.0"})
