#!/bin/bash
# run_ctf_1m_mf_energy.sh — the DNA-1M ULTIMATE TEST: the FULL `-m MF` candidate set (includes +R FreeRate, unlike
...
ALN=/scratch/dx61/sa0557/iqtree2/poc_builds/complex_data_shared/DNA/GTR+I+G4/taxa_100/len_1000000/tree_1/alignment_1000000.phy
NFULL=1000000; KSUB=5000; TOPK=3
WB="$SRC/ctf1mmfd_${ALABEL}"; mkdir -p "$WB"; cd "$WB"
[ -x "$BIN" ] && [ -f "$ALN" ] || { echo "missing binary/aln"; exit 1; }
echo "════════ CTF -m MF (ALL MODELS, incl +R) DNA-1M on ${ALABEL} — $(hostname) $(date -Iseconds) nt=$NT ════════"
nvidia-smi --query-gpu=name,memory.total,power.limit --format=csv,noheader
echo "CPU -m TEST MF baselines: np2=3076.9 np4=1974.5 np8=1443.9 np16=1122.4 s ; oracle GTR-family (GTR+I+G4 generated)"
...
# while leaving the p*ln(N) penalty fixed => it over-credits the +I/+R overfit and ranked over-parameterized models
# ABOVE the true winner (recall FAIL -> ineligible refined on CPU at 1M -> walltime). On 5000 sites every candidate is
# within <1 nat of FIT, so the rate-model choice is decided ENTIRELY by the penalty => NATIVE subsample BIC (penalty
# ln m) is the right gate. We rank ALL candidates (no pre-exclusion — red-team: don't hide the free-Q/+R coverage gap),
# add a RATE-HET DETECTOR, and cap each refine with a wall BUDGET so an ineligible CPU-refine at 1M cannot blow the
# total (PART IX X.7 / PART X X.5.5).
...
def ineligible(name):   # G.6 (free-Q DNA landed, commits 525c4186/a12e6dde/d5d69b48): the WHOLE free-Q family
    # (HKY..GTR with fixed freqs +F/+FQ) now engages JOLT on the GPU with {+G,+I+G}. So the TRUE GTR-family winner
    # of GTR-generated DNA now refines ON THE GPU, not CPU. ONLY +R/+I+R (non-mean-gamma) and pure-+I (no +G) decline
    # to CPU. (+FO would also decline but is not in the default DNA -m MF freq set.)
...
bi=min((r for r in rows if r[5]),     key=lambda r:r[2], default=None)   # best ineligible (free-Q/+R/+I)
...
if be and bi:
    ...
    lead=be[2]-bi[2]                # >0 => an ineligible (free-Q/+R/+I) model LEADS the eligible best on native BIC
    flag = lead > margin
    ...
    if flag: sys.stderr.write("  [detector] *** WARNING: a free-Q/+R/+I model genuinely leads on the subsample — eligible-refine may MISS the true winner; needs G.6 (free-Q JOLT) or CPU full-refine ***\n")
for r in nat_all[:K]:
    # refine eligible models always; refine an INELIGIBLE (free-Q/+R/+I) model only if it could plausibly win (within
    # the overfit margin of the best eligible) — else SKIP it (detector-justified, honest: we checked it cannot win),
    # which keeps the wall competitive instead of burning the full CPU budget on a doomed refine.
...
# A JOLT-eligible refine is ~78-540s on GPU; an ineligible (free-Q/+R/pure-+I) refine falls to the CPU EM optimiser at
# 1M and can run for hours (it timed out 170728179/182). `timeout` caps each model: over-budget => carried UNREFINED
# (excluded from the winner pick — its incomplete .log has no "Log-likelihood of the tree" line). The detector above
# tells us whether any unrefined ineligible model could actually be the winner (lead>margin).
...
for ENTRY in "${TOPMODELS[@]}"; do
  ...
  if [ "$ACT" = "skip" ]; then echo "  refine $i $M: SKIPPED (ineligible free-Q/+R/+I; detector: native BIC behind best eligible -> cannot win full-data)"; continue; fi
  T0=$(date +%s)
...
if best: print(f"\\nCTF -m MF WINNER: {best[0]} full lnL={best[2]:.3f} full BIC={best[1]:.1f} (oracle GTR-family)")
