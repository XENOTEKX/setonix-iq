...
int main(int argc, char** argv){
    ...
    // compute ALL non-root internal partials (postorder) + dad_eig (root excluding c0) + theta
    auto buildPartials=[&](){
        ...
        { int nch; const double* ec[3]; const double* p[3]; const unsigned char* t[3]; childArgs(T.root,c0,nch,ec,p,t); // dad = root \ c0
            k1_node<<<GB,TB>>>(nptn,NCAT,d_dad,nch,ec[0],p[0],t[0],ec[1],p[1],t[1],ec[2],p[2],t[2]); }
        k2_theta<<<GB,TB>>>(nptn,NCAT*NS,d_partial+(size_t)slot[c0]*slotSz,d_dad,d_theta);
    };
    buildPartials(); CK(cudaDeviceSynchronize()); CK(cudaGetLastError());

    // val builder + evaluator (lnL, df, ddf) at branch length t (theta cached)
    ...
    auto evalAt=[&](double t,double& lnL,double& df,double& ddf){ setVal(t);
        ...
        double L=0,kc=0; for(int p=0;p<nptn;p++){ double y=patlh[p]-kc,tt=L+y; kc=(tt-L)-y; L=tt; } // Kahan
        double D=0,DD=0; for(int p=0;p<nptn;p++){ D+=pdf[p]; DD+=pddf[p]; } lnL=L; df=D; ddf=DD; };

    // ---- (1) lnL(t0) cross-check vs K1 oracle ----
    double lnL0,df0,ddf0; evalAt(t0,lnL0,df0,ddf0);
    map<string,double> oracle={{"g4",-7541976.9391},{"r8",-7556251.9185},{"r10",-7554280.5776},{"g1",-7974816.4323}};
    printf("\n========== G.1.2 K2 derivative kernel ==========\n");
    printf("lnL(t0=%.6f) = %.4f  df=%.6e  ddf=%.6e\n",t0,lnL0,df0,ddf0);
    if (oracle.count(model)){ double o=oracle[model],rel=fabs((lnL0-o)/o);
        printf("  cross-check vs K1/G.0 oracle %.4f : rel=%.3e -> %s (freq-fold identity OK)\n",o,rel,(rel<1e-6?"PASS":"FAIL")); }
    printf("  (df at the tree's optimized length should be ~0: |df0|=%.3e)\n", fabs(df0));

    // ---- (2) FD-validate df/ddf at a t OFF the optimum (df != 0): swept-eps ----
    double tfd = t0 + 0.3;  // guaranteed OFF the optimum (~t0) so df!=0 and the FD rel-error is well-conditioned
    double lnLc,dfc,ddfc; evalAt(tfd,lnLc,dfc,ddfc);
    printf("\nFD-validation at t=%.3f (off-optimum): analytic df=%.8e ddf=%.8e\n",tfd,dfc,ddfc);
    double bestDf=1e30,bestDdf=1e30;
    for (double eps : {1e-2,1e-3,1e-4}){ double lp,lm,dummy;
        evalAt(tfd+eps,lp,dummy,dummy); evalAt(tfd-eps,lm,dummy,dummy);
        double dfFD=(lp-lm)/(2*eps), ddfFD=(lp-2*lnLc+lm)/(eps*eps);
        double rdf=fabs((dfc-dfFD)/dfFD), rddf=fabs((ddfc-ddfFD)/ddfFD);
        printf("  eps=%.0e: dfFD=%.8e (rel %.3e)  ddfFD=%.8e (rel %.3e)\n",eps,dfFD,rdf,ddfFD,rddf);
        if (rdf<bestDf) bestDf=rdf; if(rddf<bestDdf) bestDdf=rddf;
    }
    double gate = (model=="g1")?1e-6:3e-3;
    printf("  BEST: df rel=%.3e  ddf rel=%.3e  (gate df<%.0e) -> %s\n", bestDf,bestDdf,gate, (bestDf<gate?"PASS":"FAIL"));

    // ---- (3) Conditional branch optimisation: robust bisection on df=0 (df>0 below opt, df<0 above).
    // (Raw Newton diverges from a far convex-region start, ddf>0; IQ-TREE's minimizeNewton safeguards that.
    //  Here we just confirm the validated derivatives LOCATE the optimum, which should sit near t0.)
    double lo=1e-6, hi=5.0, L_,D_lo,D_hi,dd_;
    evalAt(lo,L_,D_lo,dd_); evalAt(hi,L_,D_hi,dd_);
    printf("\nConditional branch opt (bisection on df=0): df(%.0e)=%.3e  df(%.1f)=%.3e\n",lo,D_lo,hi,D_hi);
    if (D_lo>0 && D_hi<0){
        for (int it=0; it<60 && (hi-lo)>1e-9; it++){ double mid=0.5*(lo+hi),Lm,Dm,DDm; evalAt(mid,Lm,Dm,DDm); if(Dm>0) lo=mid; else hi=mid; }
        double tstar=0.5*(lo+hi),Ls,Ds,DDs; evalAt(tstar,Ls,Ds,DDs);
        printf("  t* = %.6f  lnL(t*) = %.4f  df(t*) = %.3e  ddf(t*) = %.3e\n",tstar,Ls,Ds,DDs);
        printf("  tree t0 = %.6f  lnL(t0) = %.4f   -> |t*-t0|=%.5f, ddf(t*)<0:%s, lnL(t*)>=lnL(t0):%s -> %s\n",
               t0,lnL0, fabs(tstar-t0), DDs<0?"yes":"no", Ls>=lnL0-1e-3?"yes":"no",
               (fabs(tstar-t0)<0.05 && DDs<0 && Ls>=lnL0-1e-3)?"PASS (derivatives locate the optimum near t0)":"CHECK");
    } else printf("  bracket sign check failed\n");

    // ---- timing (full derivative eval: one k2_derv launch + reductions) ----
    double best=1e30; for(int r=0;r<reps;r++){ auto a=Clock::now(); double L,d,dd; evalAt(t0,L,d,dd); best=min(best,now_ms(a,Clock::now())); }
    printf("\nderv eval (incl val upload + reduce + D2H): min %.3f ms (%d reps)\n",best,reps);
    printf("================================================\n");
