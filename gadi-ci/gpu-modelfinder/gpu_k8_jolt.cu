...
int main(int argc, char** argv){
    if (argc<3){ fprintf(stderr,"usage: %s <aln.phy> <tree> [model=g4|g1] [maxiter]\n",argv[0]); return 1; }
    string alnpath=argv[1],treepath=argv[2]; string model=(argc>3)?argv[3]:"g4"; int maxiter=(argc>4)?atoi(argv[4]):300;

    // ===================== alignment via MMAP (RAM-resident page cache; disk removed from the load path) =====================
    // The file pages are mapped into RAM and parsed by pointer-walk; tip states stage through a PINNED host buffer
    // (cudaHostAlloc) for a ~2-3x-faster H2D. HONEST SCOPE (advisor): this is the ONE-TIME load lever — the hot
    // optimiser loop is dependent-kernel-bound and touches no disk and only a tiny per-iter echild H2D. The async
    // double-buffered RAM->GPU streaming win belongs to the 1M/10M TILING regime (G.4.3), not here.
    ...
    char* amap=(char*)mmap(nullptr,aln_bytes,PROT_READ,MAP_PRIVATE,afd,0);
    if(amap==MAP_FAILED){fprintf(stderr,"mmap failed\n");return 2;}
    madvise(amap,aln_bytes,MADV_WILLNEED);
    int ntax=0,nsite=0; const char* pc=amap; const char* pend=amap+aln_bytes;
    { sscanf(pc,"%d %d",&ntax,&nsite); while(pc<pend&&*pc!='\n')pc++; if(pc<pend)pc++; }
    vector<string> seqs(ntax); map<string,int> name2tip;
    for(int t=0;t<ntax;t++){ const char* ls=pc; while(pc<pend&&*pc!='\n')pc++; string line(ls,(size_t)(pc-ls)); if(pc<pend)pc++;
        istringstream is(line); string nm,sq; is>>nm>>sq; seqs[t]=sq; name2tip[nm]=t; }
    int nptn=nsite;
    printf("[aln] ntax=%d nsite=%d  (mmap %.1f MB, RAM-resident page cache; disk out of load path)\n",ntax,nptn,aln_bytes/1048576.0);

    // ---- tree: topology FIXED; .treefile branch lengths kept as the MLE reference, brlen COLD-STARTED below ----
    ...
    // ---- model: rates FIXED at the validated MLE (g4 = LG+G4 MLE rates); G.4.1 optimises BRANCHES jointly ----
    ...
    else { fprintf(stderr,"G.4.1 gate uses g4 (LG+G4, the load-bearing +G case) or g1; +R/joint-alpha = G.4.1b\n"); return 1; }
    printf("[model] %s NCAT=%d (alpha/rates fixed at MLE; joint branch optimisation)\n",model.c_str(),NCAT);

    // ---- eigendecomposition (IQ-TREE reversible convention; identical to K1/K7) ----
    ...
    // ---- postorder slots, MLE brlen reference (childLen), edge list, central edge, tree height ----
    ...
    vector<int> edgeV; for(int u=0;u<nnodes;u++) for(int v:T.nodes[u].child) edgeV.push_back(v);
    int nedge=edgeV.size();
    ...
    printf("[edges] %d branches (2N-3); central=(root=%d,c0=%d); tree height=%d, pre-pool=%d slots\n",nedge,T.root,c0,treeH,nPool);

    size_t ecStride=(size_t)NCAT*NS*NS, slotSz=(size_t)NCAT*NS*nptn;

    // ---- PINNED host staging: tips (mmap->pinned->GPU) + per-iter echild/expfac (pinned for fast H2D) ----
    ...
    munmap(amap,aln_bytes); close(afd);           // alignment fully consumed into host structures + device tips
    ...
    double *d_echild,*d_partial,*d_theta,*d_patlh,*d_pdf,*d_pddf,*d_pretmp,*d_tipeig,*d_expfac,*d_prepool;
    ...
    printf("[mem] partials %.2f GB + pre-pool %.2f GB ; tip+echild H2D via PINNED host (mmap->pinned->GPU)\n",
        (double)nInternal*slotSz*8/1073741824.0,(double)nPool*slotSz*8/1073741824.0);
    ...
    // rebuild echild + expfac from `brlen` (host, pinned H2D). echild[c][cat][x][i]=U[x][i]*exp(eval[i]*rate_c*brlen[c]);
    // expfac[c][cat][i]=exp(eval[i]*rate_c*brlen[c]) (k7_pre parent-branch factor) -- the SAME ex[i].
    auto rebuildEchild=[&](){
    ...
    long nGradSweeps=0, nLnLEval=0;                  // critical-path instrumentation (the JOLT thesis metric)
    // full lnL at a candidate brlen: rebuild echild + postorder + central-edge reduction (edge-invariant)
    auto evalLnL=[&](const vector<double>& cand)->double{ brlen=cand; rebuildEchild(); postorderFill();
    ...
    // gradient: O(depth) recycled preorder -> df_e, ddf_e for ALL edges in ONE postorder+preorder sweep; returns lnL.
    vector<double> g_df(nedge,0.0), g_ddf(nedge,0.0);
    auto computeGradient=[&](double& lnLout){
        rebuildEchild(); postorderFill(); nGradSweeps++;
        vector<int> freeSlots; for(int s=nPool-1;s>=0;s--) freeSlots.push_back(s);
        auto acq=[&](){int s=freeSlots.back();freeSlots.pop_back();return s;}; auto rel=[&](int s){freeSlots.push_back(s);};
        ...
        function<void(int,int)> proc=[&](int u,int su){
            for(int v:T.nodes[u].child){
                ...
                setVal(brlen[v]); k2_derv<<<GB,TB>>>(nptn,NCAT,d_theta,d_patlh,d_pdf,d_pddf); CK(cudaDeviceSynchronize());
                double l,d,dd; reduceDerv(l,d,dd); dfC[v]=d; ddfC[v]=dd; if(!gotL){lnLfirst=l;gotL=true;}
                if(T.nodes[v].leaf<0) proc(v,sv);
                rel(sv);
            } };
        proc(T.root,-1); CK(cudaDeviceSynchronize());
        for(int e=0;e<nedge;e++){ g_df[e]=dfC[edgeV[e]]; g_ddf[e]=ddfC[edgeV[e]]; } lnLout=lnLfirst; };

    map<string,double> oracle={{"g4",-7541976.9391},{"g1",-7974816.4323}};
    double O=oracle.count(model)?oracle[model]:0.0;

    // ===================== PRE-CHECK (advisor #4): assembly + sign at the MLE branches =====================
    brlen=mleLen; double lnL_mle; computeGradient(lnL_mle);
    double gnorm_mle=0; for(int e=0;e<nedge;e++) gnorm_mle+=g_df[e]*g_df[e]; gnorm_mle=sqrt(gnorm_mle);
    double rel_mle=O?fabs((lnL_mle-O)/O):0;
    printf("\n========== G.4.1 PRE-CHECK (gradient assembly + sign at the MLE branches) ==========\n");
    printf("lnL(.treefile)=%.6f vs oracle %.6f rel=%.3e %s ; ||g_b||=%.4e (the CPU-optimum gradient floor = calibration)\n",
        lnL_mle,O,rel_mle,(rel_mle<=1e-9?"PASS":"WARN"),gnorm_mle);

    // ===================== joint LM diagonal-Newton; run WARM (.treefile) then COLD (b=0.1) =====================
    struct OptRes{ double lnL; int iters; long sweeps; long evals; int rejects; double gnorm; };
    vector<double> cand(nnodes,0.0);
    auto optimize=[&](vector<double> start,const char* tag)->OptRes{
        long sw0=nGradSweeps, ev0=nLnLEval; brlen=start; double lnL=evalLnL(brlen); nLnLEval++;
        double mu=1.0, tol=1e-7; int it=0,nRej=0; bool conv=false; double gnorm=0; double startL=lnL;
        printf("\n---- optimise [%s]: start lnL=%.6f (rel %.3e) ----\n",tag,lnL,O?fabs((lnL-O)/O):0);
        for(it=1; it<=maxiter; it++){
            vector<double> base=brlen; double lg; computeGradient(lg);   // df/ddf at base
            gnorm=0; for(int e=0;e<nedge;e++) gnorm+=g_df[e]*g_df[e]; gnorm=sqrt(gnorm);
            bool acc=false;
            for(int bt=0; bt<12; bt++){                                  // LM accept/reject (no line search to balloon)
                cand=base; for(int e=0;e<nedge;e++){ int v=edgeV[e]; double dn=fabs(g_ddf[e])+mu;   // ddf = diagonal preconditioner
                    double nb=base[v]+g_df[e]/dn; if(nb<1e-6)nb=1e-6; if(nb>20.0)nb=20.0; cand[v]=nb; }
                double ln=evalLnL(cand); nLnLEval++;
                if(ln>lnL+1e-9){ double dl=ln-lnL; brlen=cand; lnL=ln; mu=fmax(mu*0.5,1e-9); acc=true; if(dl<tol)conv=true; break; }
                else { mu*=4.0; nRej++; } }
            if(!acc){ brlen=base; printf("  [%s it %d] no uphill step (mu=%.2e) -> stop\n",tag,it,mu); break; }
            if(it<=6||it%25==0||conv) printf("  [%s it %3d] lnL=%.6f rel=%.3e ||g||=%.3e mu=%.2e rej=%d\n",
                tag,it,lnL,O?fabs((lnL-O)/O):0,gnorm,mu,nRej);
            if(conv) break;
        }
        printf("  [%s] DONE: %d iters, dlnL=%.4f (%.6f->%.6f), %ld grad-sweeps + %ld lnL-evals, %d rejects, final||g||=%.3e\n",
            tag,it,lnL-startL,startL,lnL,nGradSweeps-sw0,nLnLEval-ev0,nRej,gnorm);
        OptRes r; r.lnL=lnL; r.iters=it; r.sweeps=nGradSweeps-sw0; r.evals=nLnLEval-ev0; r.rejects=nRej; r.gnorm=gnorm; return r; };

    OptRes warm=optimize(mleLen,"WARM .treefile");
    vector<double> coldVec(nnodes,0.1);
    OptRes cold=optimize(coldVec,"COLD b=0.1");

    // ===================== VERDICT =====================
    double relCW = warm.lnL!=0 ? fabs((cold.lnL-warm.lnL)/warm.lnL) : fabs(cold.lnL-warm.lnL);
    double relCO = O ? fabs((cold.lnL-O)/O) : 0;
    double brl_worst=0; for(int e=0;e<nedge;e++){ int v=edgeV[e]; double r=mleLen[v]>1e-6?fabs((brlen[v]-mleLen[v])/mleLen[v]):0; if(r>brl_worst)brl_worst=r; }
    printf("\n========== G.4.1 VERDICT [%s] joint LM diagonal-Newton ==========\n",model.c_str());
    printf("(1) COLD reaches the SAME optimum as WARM: cold lnL=%.6f, warm lnL=%.6f, rel=%.3e -> %s (gate 1e-9; cold-start is the meaningful test)\n",
        cold.lnL,warm.lnL,relCW,(relCW<=1e-9?"PASS":"FAIL"));
    printf("    cold lnL vs .treefile oracle %.6f: rel=%.3e (>=oracle means JOLT found the branch-MLE at these fixed rates)\n",O,relCO);
    printf("(2) HEADLINE (the JOLT thesis metric) — COLD start, %d branches optimised JOINTLY:\n",nedge);
    printf("    %d joint iterations ; %ld gradient sweep-pairs + %ld lnL evals = %ld dependent full-tree traversals on the critical path\n",
        cold.iters,cold.sweeps,cold.evals,cold.sweeps*2+cold.evals);
    printf("    final ||g||=%.3e (vs CPU-optimum floor %.3e) ; %d backtrack-rejects ; worst brlen rel vs MLE %.2e (same basin)\n",
        cold.gnorm,gnorm_mle,cold.rejects,brl_worst);
    printf("(3) CONTRAST: IQ-TREE optimizeAllBranches visits ALL %d branches SEQUENTIALLY (Gauss-Seidel, each edge reads the\n",nedge);
    printf("    previous edge's fresh partials) x ~2-5 Newton steps x several sweeps. JOLT updates ALL %d branches from ONE\n",nedge);
    printf("    parallel preorder sweep/iter -> the critical path is %d iters, NOT %d-sequential-edges-deep.\n",cold.iters,nedge);
    printf("    (warm-start took %d iters; cold is the honest cold-start headline. vs-IQ-TREE step count is approximate:\n",warm.iters);
    printf("     IQ-TREE warm-starts each candidate from BIONJ, not cold — report the absolute JOLT iteration count.)\n");
    bool PASS = (relCW<=1e-9) && (cold.lnL>=O-1e-3);
    printf("VERDICT [%s]: %s\n", model.c_str(), PASS?"PASS — joint parallel optimiser converges to the same MLE from a cold start":"FAIL — investigate convergence");
    printf("=========================================================\n");
    ...
    cudaFree(d_tip);cudaFree(d_pretmp);cudaFree(d_tipeig);cudaFree(d_expfac);cudaFree(d_prepool);
    return 0;
}
