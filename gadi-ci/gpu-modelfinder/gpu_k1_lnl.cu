// gpu_k1_lnl.cu — Phase G.1.1: custom in-tree-style CUDA postorder log-likelihood kernel (K1).
//
// Standalone harness that computes the LG+G/+R log-likelihood of a FIXED tree + AA alignment with a
// CUSTOM CUDA eigen-space postorder kernel (NOT BEAGLE), validated against the G.0 oracle. This is the
// first NUMERICAL phase of gpu-modelfinder-design.md PART II. The CPU-side scaffolding (LG matrix,
// reversible eigendecomposition -> U/Lambda/U^-1, Newick parser, compact tip states, gamma/FreeRate
// rates) is lifted verbatim from the validated G.0 harness gpu_derisk.cpp; only the compute is new.
//
// MATH (eigen-space, reproduces IQ-TREE's computePartialLikelihoodSIMD exactly; lnL is
// representation-independent so it must match the G.0 BEAGLE oracle):
//   leaf state s:   L[c][i] = Uinv[i][s]            (column s of U^-1; unknown -> row-sum over s)
//   internal node:  pk[x]   = sum_i echild_k[c][x][i] * L_child_k[c][i]   (per child, prob space)
//                   prod[x] = prod_k pk[x]
//                   L[c][r] = sum_x Uinv[r][x] * prod[x]   (back to eigen space)
//   root node:      lh_ptn  = sum_c prop_c * sum_x freq[x] * prod_root_c[x]   (no U^-1)
//                   lnL     = sum_ptn log(lh_ptn)          (all sites, weight 1 -> matches oracle setup)
//   echild_k[c][x][i] = U[x][i] * exp(eval[i] * rate_c * len_k)             (precomputed on host)
// FP64, UNSCALED (AA-100K/100-taxa never underflows -> bit-parity with CPU NORM_LH path, G.0 finding 6).
// NCAT loops on the kernel's category dimension -> NO BEAGLE kMatrixBlockSize<=8 cap, so +R10 runs in
// ONE pass.
//
// Build (in a gpuvolta PBS job): module load cuda/12.5.1 gcc/12.2.0
//   nvcc -O3 -std=c++17 -arch=sm_70 gpu_k1_lnl.cu -o gpu_k1_lnl
// Run:  ./gpu_k1_lnl <aln.phy> <tree.treefile> [model=g4|r8|r10|g1|fig4] [reps]
//
...
// ===================== CPU-side scaffolding (lifted from gpu_derisk.cpp, BEAGLE-free) =====================
...
static int aa_index(char c) {
    ...
    return -1; // B,Z,J,X,-,?,* -> fully ambiguous (matches G.0 oracle)
}
...
__device__ __forceinline__ void accum_child(double* prod, int c, int ptn, int nptn,
    ...
    } else {                              // leaf child: L[i] = column s of U^-1 (or row-sum if ambiguous)
        int s = t[ptn];
        #pragma unroll
        for (int x=0;x<NS;x++){ double v=0.0;
            #pragma unroll
...
int main(int argc, char** argv){
    if (argc<3){ fprintf(stderr,"usage: %s <aln.phy> <tree> [model=g4|r8|r10|g1|fig4] [reps]\n",argv[0]); return 1; }
    ...
    int nptn=nsite;
    printf("[aln] ntax=%d nsite=%d (all sites, weight 1)\n", ntax, nptn);
    double fEmp[20]; { long cnt[20]={0},tot=0; for(int t=0;t<ntax;t++) for(char c:seqs[t]){int a=aa_index(c); if(a>=0){cnt[a]++;tot++;}}
        for(int k=0;k<20;k++) fEmp[k]=tot?(double)cnt[k]/tot:0.05; }
    ...
    printf("[tree] nodes=%d root_children=%d\n", nnodes, rootdeg);

    // ---- model: freqs + category rates/weights (lifted from gpu_derisk) ----
    double R[20][20], f[20]; fill_LG(R,f);
    int NCAT=4; vector<double> catRates, catWeights;
    bool hasF=(model.find('f')!=string::npos);
    if (hasF) for(int k=0;k<20;k++) f[k]=fEmp[k];
    ...
    else if (model=="fig4"){ double pinv=0.1,g[4]={0.1362,0.4756,0.9994,2.3887}; NCAT=5; catRates={0.0,g[0],g[1],g[2],g[3]};
        double wg=(1.0-pinv)/4.0; catWeights={pinv,wg,wg,wg,wg}; double mean=0; for(int k=0;k<NCAT;k++)mean+=catWeights[k]*catRates[k]; for(int k=0;k<NCAT;k++)catRates[k]/=mean; }
    ...
    // ---- LG reversible eigendecomposition (U/Lambda/U^-1, IQ-TREE convention) ----
    double Q[20][20];
    ...
    printf("[eig] eval[0..3]= %.6f %.6f %.6f %.6f (one ~0 expected)\n", evals[0],evals[1],evals[2],evals[3]);

    // ---- internal-node postorder + slot assignment; per-child edge lengths ----
    ...
    // ---- host echild[child_node][cat][x][i] = U[x][i]*exp(eval[i]*rate_c*len_child) ----
    size_t ecStride = (size_t)NCAT*NS*NS;                 // per child node
    vector<double> echild((size_t)nnodes*ecStride, 0.0);
    for (int c=0;c<nnodes;c++){ if(c==T.root) continue;   // root has no parent edge
        for (int cat=0;cat<NCAT;cat++){ double len=childLen[c]*catRates[cat];
    ...
    size_t partialBytes=(size_t)nInternal*slotSz*sizeof(double);
    printf("[mem] partials arena = %.2f GB (%d slots x %zu doubles)\n", partialBytes/1073741824.0, nInternal, slotSz);
    CK(cudaMalloc(&d_partial, partialBytes));
    CK(cudaMalloc(&d_patlh, (size_t)nptn*sizeof(double)));

    // ---- per-internal-node child descriptors (host) ----
    ...
    for (int idx=0; idx<nInternal; idx++){ int u=postorder[idx]; Desc& D=desc[idx];
        D.isRoot=(u==T.root)?1:0; D.nchild=T.nodes[u].child.size(); if(D.nchild>3){fprintf(stderr,"node deg %d >3 unsupported\n",D.nchild);return 6;}
    ...
    auto evalOnce=[&](){
        for (int idx=0; idx<nInternal; idx++){ Desc& D=desc[idx];
            k1_node<<<GB,TB>>>(nptn,NCAT,D.isRoot,D.out,d_patlh,D.nchild,
                D.ec[0],D.p[0],D.t[0], D.ec[1],D.p[1],D.t[1], D.ec[2],D.p[2],D.t[2]);
        }
    };
    // warmup + correctness
    evalOnce(); CK(cudaDeviceSynchronize()); CK(cudaGetLastError());
    vector<double> patlh(nptn);
    CK(cudaMemcpy(patlh.data(), d_patlh, nptn*sizeof(double), cudaMemcpyDeviceToHost));
    double lnL=0.0, kc=0.0; // Kahan
    for (int p=0;p<nptn;p++){ double y=patlh[p]-kc, t=lnL+y; kc=(t-lnL)-y; lnL=t; }
    double lnL_plain=0.0; for(int p=0;p<nptn;p++) lnL_plain+=patlh[p];

    // timing
    double best=1e30,sum=0;
    for (int r=0;r<reps;r++){ auto t0=Clock::now(); evalOnce(); CK(cudaDeviceSynchronize()); double ms=now_ms(t0,Clock::now()); best=min(best,ms); sum+=ms; }

    // ---- oracle compare ----
    map<string,double> oracle={{"g4",-7541976.9391},{"r8",-7556251.9185},{"r10",-7554280.5776},{"g1",-7974816.4323},{"fig4",-7551774.2140}};
    printf("\n========== G.1.1 K1 result (custom CUDA eigen-space postorder) ==========\n");
    printf("model=%s  NCAT=%d  lnL(Kahan)= %.4f  lnL(plain)= %.4f\n", model.c_str(), NCAT, lnL, lnL_plain);
    if (oracle.count(model)){ double o=oracle[model]; double rel=fabs((lnL-o)/o);
        printf("oracle(G.0 BEAGLE)= %.4f   |dlnL|= %.4e   rel= %.3e   -> %s\n", o, fabs(lnL-o), rel, (rel<1e-6?"PASS":"FAIL")); }
    printf("lnL eval: min %.3f ms, mean %.3f ms (%d reps)\n", best, sum/reps, reps);
