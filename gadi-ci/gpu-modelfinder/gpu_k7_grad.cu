// gpu_k7_grad.cu — Phase G.4.0 (JOLT): custom CUDA preorder ALL-BRANCH gradient kernel (K7).
// Extends the validated K1 (postorder lnL) + K2 (single-edge df/ddf) harness with ONE new kernel, k7_pre,
// a top-down preorder sweep that yields the eigen-space "rest of tree" partial pre_v for EVERY node. The
// per-edge gradient is then theta_e = pre_v (.) pl_v fed through the BIT-VALIDATED k2_derv reduction, so all
// 2N-3 branch derivatives come from ONE postorder + ONE preorder sweep (Ji-2020 O(N) gradient). Gate: for
// every edge, lnL_e == K1 oracle rel<=1e-9 (validates pre_v via the freq-fold identity) AND df FD-validated;
// the central edge reproduces the G.1.2/G.2.1a-validated K2 df. Build: nvcc -O3 -std=c++17 -arch=sm_70.
...
int main(int argc, char** argv){
    ...
    // ===================== G.4.0 — K7 preorder ALL-BRANCH gradient =====================
    // The all-branch gradient is df_e = Sum_ptn ( Sum_c Sum_i val1[c][i]*theta_e[c][i] ) with
    // theta_e = pre_v (.) pl_v.  pl_v = postorder partial (subtree below v); pre_v = preorder partial
    // (rest of tree).  The val0/val1/val2 reduction (k2_derv) is the SAME one validated bit-identically in
    // G.2.1a/b — only pre_v is new (k7_pre).  DE-RISK: if pre_v is correct, lnL_e is EDGE-INVARIANT and ==
    // the K1 oracle for EVERY edge (freq-fold identity), and df_e FD-validates exactly as the K2 central edge
    // did.  NOTE: this validation harness uses a naive one-pre-slot-per-node buffer; production JOLT uses
    // Ji-2020 O(depth) recycling (g0-log:227) — out of scope for this correctness gate.

    double *d_pre,*d_tipeig,*d_expfac;
    CK(cudaMalloc(&d_pre,(size_t)nnodes*slotSz*sizeof(double)));
    CK(cudaMalloc(&d_tipeig,slotSz*sizeof(double)));
    vector<double> expfac((size_t)nnodes*NCAT*NS,0.0);          // exp(eval[i]*rate_c*childLen[node])
    ...
    printf("[mem] +preorder %.2f GB (naive 1/node; production uses O(depth) recycle)\n",(double)nnodes*slotSz*8/1073741824.0);

    // ---- postorder: ALL non-root internal partials ----
    ...
    // ---- preorder: seed root children (pre = partial of root EXCLUDING that child), then top-down k7_pre ----
    ...
    function<void(int)> pre_dfs=[&](int u){
        for(int v:T.nodes[u].child){
            if(u!=T.root){                                  // root children already seeded
                const double* ec[2]={nullptr,nullptr}; const double* sp[2]={nullptr,nullptr}; const unsigned char* st[2]={nullptr,nullptr}; int ns=0;
                for(int w:T.nodes[u].child){ if(w==v||ns>=2) continue; sibArg(w,ec[ns],sp[ns],st[ns]); ns++; }
                k7_pre<<<GB,TB>>>(nptn,NCAT,d_pre+(size_t)v*slotSz,d_pre+(size_t)u*slotSz,d_expfac+(size_t)u*NCAT*NS,
                                  ns,ec[0],sp[0],st[0],ec[1],sp[1],st[1]);
            }
            if(T.nodes[v].leaf<0) pre_dfs(v);
        }
    };
    pre_dfs(T.root); CK(cudaDeviceSynchronize()); CK(cudaGetLastError());
    ...
    auto setVal=[&](double t){ vector<double> v0(NCAT*NS),v1(NCAT*NS),v2(NCAT*NS);
        for(int c=0;c<NCAT;c++){ double rc=catRates[c],pc=catWeights[c]; for(int x=0;x<NS;x++){ double re=rc*evals[x],e=exp(evals[x]*rc*t)*pc;
    ...
    auto edgeTheta=[&](int v){ const double* pl;
        ...
        k2_theta<<<GB,TB>>>(nptn,NCAT*NS,pl,d_pre+(size_t)v*slotSz,d_theta); };

    // ---- validate: every edge lnL_e == oracle (validates pre_v) AND df FD-validated ----
    printf("\n========== G.4.0 K7 preorder all-branch gradient ==========\n");
    ...
    double worstLnL=0,worstDf=0,gradNorm=0; int nInt=0,nLeaf=0;
    double dfGate=(model=="g1")?1e-6:3e-3, eps=1e-3;
    int cedge=-1; double cdf=0;        // central edge (root->c0) ties to the G.1.2/G.2.1a-validated K2 df
    vector<double> grad; vector<int> gv;
    for(int u=0;u<nnodes;u++) for(int v:T.nodes[u].child){
        double bv=childLen[v];
        edgeTheta(v); CK(cudaDeviceSynchronize()); CK(cudaGetLastError());
        double le,dfe,ddfe; evalAt(bv,le,dfe,ddfe);
        double rl=O?fabs((le-O)/O):0.0; if(rl>worstLnL) worstLnL=rl;
        // FD-validate the df FORMULA at an OFF-optimum length: at the tree's bv, df~0 (optimized) so the
        // relative FD error is ill-conditioned (divide-by-~0). theta_e is t-independent, so evaluate df at
        // tfd=bv+0.2 (well into df!=0) and central-difference there — exactly the K2 off-optimum discipline.
        double tfd=bv+0.2, lc,dft,d3; evalAt(tfd,lc,dft,d3);
        double rdf=1e30;                                  // swept-eps central difference, best of {1e-2,1e-3,1e-4}
        for(double e:{1e-2,1e-3,1e-4}){ double lp,lm; evalAt(tfd+e,lp,d3,d3); evalAt(tfd-e,lm,d3,d3);
            double dfFD=(lp-lm)/(2*e); double r=fabs(dfFD)>1e-3?fabs((dft-dfFD)/dfFD):fabs(dft-dfFD);
            if(r<rdf) rdf=r; }
        if(rdf>worstDf) worstDf=rdf;
        grad.push_back(dfe); gv.push_back(v); gradNorm+=dfe*dfe;
        if(u==T.root && v==c0){ cedge=v; cdf=dfe; }
        if(T.nodes[v].leaf<0) nInt++; else nLeaf++;
    }
    gradNorm=sqrt(gradNorm);
    printf("edges: %d internal + %d leaf = %d (all derivatives from ONE preorder sweep)\n",nInt,nLeaf,nInt+nLeaf);
    printf("(1) lnL edge-invariance vs oracle %.4f: worst rel=%.3e -> %s  (gate 1e-9; VALIDATES pre_v)\n",
           O,worstLnL,(worstLnL<=1e-9?"PASS":"FAIL"));
    printf("(2) all-branch df FD-validation: worst rel=%.3e -> %s  (gate %.0e)\n",
           worstDf,(worstDf<dfGate?"PASS":"FAIL"),dfGate);
    if(cedge>=0) printf("(3) central edge (root->c0=%d): df=%.6e  (matches the G.1.2/G.2.1a-validated K2 path)\n",cedge,cdf);
    printf("|grad|_2=%.6e over %zu branches; sample:",gradNorm,grad.size());
