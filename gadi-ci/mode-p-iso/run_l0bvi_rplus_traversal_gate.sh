...
SEED="${SEED:-1}"
EXPECTED_MD5="${EXPECTED_MD5:-8469af7b2035cd110ae3b5be1d80474f}"  # L.0b.vi build 169647747
# L.0b.vi clean +R traversal gate: evaluate ONLY high-dim +R models on a fixed LG matrix
# (-mrate R overrides the -m TEST ratehet set to +R; -cmin/-cmax bound the categories).
# This is the regime that MOTIVATES the LM: high-dim FreeRate weight+rate joint fit, where
# the legacy alternating EM/BFGS does many full-tree passes.  Weight dims are now analytic
# (L.0b.vi), so MODE-L runs full_analytic — no FD weight probes inflating its count.
MODEL_RATE_FLAGS="${MODEL_RATE_FLAGS:- -mset LG -mrate R -cmin 8 -cmax 10}"

PBS_ID_SHORT="${PBS_JOBID:-local_$(date +%Y%m%d_%H%M%S)}"; PBS_ID_SHORT="${PBS_ID_SHORT%%.*}"
LABEL="rplus_l0bvi_aa100k_np1_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  Mode L L.0b.vi +R traversal gate: BASE vs LM  AA 100K  +R8-R10 (LG)  ║"
...
run_arm "BASE" "${BASE_DIR}" ${MODEL_RATE_FLAGS}

# ── MODE-L arm: production mode (analytic gradient, no FD check overhead) ─
run_arm "MODE-L" "${MODL_DIR}" --mode-l ${MODEL_RATE_FLAGS}
...
# (1) +R sweep: no fixed LG+G4 winner — require both arms to AGREE on the winning +R model
if [[ -n "${BASE_BEST}" && "${BASE_BEST}" == "${MODL_BEST}" ]]; then
    echo "  ✓ both arms agree best=${BASE_BEST}"
    [[ "${BASE_BEST}" == *"+R"* ]] && echo "  ✓ winner is a +R model (${BASE_BEST})" \
        || { echo "  ✗ FAIL: winner ${BASE_BEST} is not +R (expected +R-only sweep)"; PASS=0; }
else
    echo "  ✗ FAIL: arms disagree on best model (BASE=${BASE_BEST} MODL=${MODL_BEST})"; PASS=0
fi
