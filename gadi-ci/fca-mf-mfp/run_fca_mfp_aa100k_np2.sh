#!/bin/bash
# run_fca_mfp_aa100k_np2.sh — FCA -m MFP parity: np=1 baseline + np=2 parallel, AA 100K.
#
# PURPOSE: Verify -m MFP (FreeRate + mixture models) FCA parity at np=2 vs np=1.
...
LABEL="fca_mfp_aa100k_np2_par_seed${SEED}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  FCA -m MFP parity: AA 100K  np=1 baseline + np=2 parallel   ║"
...
echo ""
echo "══ Parity: FCA -m ${MODEL_FLAG} AA 100K np=2 vs np=1 ══════════════════════"
echo "  BASE (np=1): rc=${BASE_RC}  lnL=${BASE_LNL}  best=${BASE_BEST}  MF=${BASE_MFW}s"
echo "  FCA  (np=2): rc=${FCA_RC}   lnL=${FCA_LNL}   best=${FCA_BEST}   MF=${FCA_MFW}s"
PASS=1
