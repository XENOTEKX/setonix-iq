#!/bin/bash
# run_mega_mf2_full_omp_batch.sh — MF2 Full IQ-TREE OMP single-node
# scaling batch on mega_dna.fa.  Runs 64T and 104T sequentially in one
# PBS job on a single Gadi normalsr SPR node.
#
# Parity series with run_mega_avx512_r2_omp_batch.sh (same dataset/seed/protocol)
# and with the xlarge_mf MF2 Full OMP series (same binary/build_tag):
...
#   project: um09
#
# Estimated walltime (mega_dna AVX-512+R2 64T≈1826s, 104T≈1022s expected):
#   64T ≈ 1800 s,  104T ≈ 1400 s  → ≈ 3400 s total; 8 h requested
#
# build_tag: mf2_full_icx_avx512_r2_lpt
#
#PBS -N iq-mega-mf2-omp
#PBS -P um09
#PBS -q normalsr
#PBS -l ncpus=104
#PBS -l mem=500GB
...
for THREADS in "${THREAD_COUNTS[@]}"; do
    LABEL="${DATASET_NAME}_${THREADS}t_mf2_full_np1_seed1"
    RUN_ID="gadi_${LABEL}"
    ...
    echo ""
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║  MF2 Full IQ-TREE  —  single-node OMP  —  ${THREADS} threads"
    echo "║  run_id:    ${RUN_ID}"
...
SAMPLER_EOF
    ...
    # Pass 1 — clean timing
    echo "[mf2-full-omp] Pass 1: ${THREADS}T timing"
...
iTLB-loads,iTLB-load-misses"
    ...
    echo "[mf2-full-omp] ${THREADS}T Pass 1 done: rc=${IQRC} wall=${WALL}s"

    # Pass 2 — perf stat
    if [[ "${IQRC}" -eq 0 ]] && command -v perf >/dev/null 2>&1; then
        echo "[mf2-full-omp] ${THREADS}T Pass 2: perf stat"
        mpirun -np 1 \
            -x "OMP_NUM_THREADS=${THREADS}" \
            -x "OMP_DYNAMIC=false" \
            -x "OMP_PROC_BIND=close" \
...
record = {
    ...
    "description": f"Gadi SPR MF2 Full — single-node {threads}T OMP on mega_dna.fa",
    "timing": [{
...
print(f"[mf2-full-omp] wrote {out_path}")
PYEOF
...
echo ""
echo "[mf2-full-omp] batch complete: OVERALL_RC=${OVERALL_RC}"
exit "${OVERALL_RC}"
