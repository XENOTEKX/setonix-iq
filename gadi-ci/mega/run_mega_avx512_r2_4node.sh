#!/bin/bash
# run_mega_avx512_r2_4node.sh — ICX + AVX-512 + R2 MPI 4-node
...
#   project: um09
#
# build_tag: icx_mpi4x104_4node_avx512_r2
#
#PBS -N iq-mega-avx512-r2-4node
#PBS -P um09
#PBS -q normalsr
#PBS -l ncpus=416
#PBS -l mem=2000GB
...
SEED="${SEED:-1}"
LABEL="${LABEL:-${DATASET_NAME}_${TOTAL_THREADS}t_icx_mpi${NRANKS}x${OMP_PER_RANK}_4node_avx512_r2}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  IQ-TREE R2 + AVX-512 — MPI 4-node full-node placement"
echo "║  run_id:        ${RUN_ID}"
...
record = {
    ...
    "description": f"Gadi SPR R2+AVX-512 — MPI 4-node {nranks}×{omp_per_rank} = {threads}T on mega_dna.fa",
    "timing": [{
