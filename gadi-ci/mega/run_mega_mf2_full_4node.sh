#!/bin/bash
# run_mega_mf2_full_4node.sh — MF2 Full IQ-TREE MPI 4-node
# on mega_dna.fa: 4 MPI ranks × 104 OpenMP threads = 416 total cores.
#
# Parity series with run_mega_avx512_r2_4node.sh (same dataset/seed/protocol)
# and with the xlarge_mf MF2 Full 4-node series (same binary/build_tag):
...
#   project: um09
#
# Estimated walltime (mega_dna AVX-512+R2 416T≈606s; extra MF2 dispatch overhead):
#   416T ≈ 700–900 s  → 3 h requested
#
# build_tag: mf2_full_icx_avx512_r2_lpt
#
#PBS -N iq-mega-mf2-4node
#PBS -P um09
#PBS -q normalsr
#PBS -l ncpus=416
#PBS -l mem=2000GB
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  MF2 Full IQ-TREE — MPI 4-node full-node placement"
echo "║  run_id:        ${RUN_ID}"
...
echo "[4node] head node topology: sockets=${LSCPU_SOCKETS} cores/socket=${LSCPU_COREPS} → physical_cores=${PHYSICAL_CORES}"
...
if [[ "${IQRC}" -eq 0 ]] && command -v perf >/dev/null 2>&1; then
    echo "[4node] Pass 2: perf stat (per-rank)"
    mpirun -np "${NRANKS}" \
        --hostfile "${HOSTFILE}" \
        --mca rmaps_base_mapping_policy "" \
        -rf "${RANKFILE}" \
...
record = {
    ...
    "description": f"Gadi SPR MF2 Full — MPI 4-node {nranks}×{omp_per_rank} = {threads}T on mega_dna.fa",
    "timing": [{
