#!/bin/bash
# run_ws_a2_aa_1m_16node_full.sh — Full MF+SPR parity run: warm-start A.2 binary, AA 1M, 16-node MPI.
#
...
#   MF=1,122.363s SPR=1,287.863s total=2,410.226s) and WS-A.1 np=16 (job 169095645,
#   MF=1,146.174s SPR=1,212.944s total=2,440.301s) to measure Phase A.2 cross-rank MPI
#   broadcast benefit at scale.  Phase A.2 broadcasts the accepted rate vector from rank 0
#   to all ranks after filterRatesMPI fires (ws_bcast_fields > 0 confirms the path).
#
# Gate (§5.7 W4): AA 1M np=16 full run.
#   Pass criteria:
#     - exit code = 0
...
# Related:   CHANGELOG (cc)/(ce) WS-A.2, research/lbfgs-and-warmstart-implementation.md §12.8

#PBS -N ws-a2-aa-1m-16n
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=1664
#PBS -l mem=8160GB
...
LABEL="AA_1m_ws_a2_np16_full_seed${SEED}"
...
# Phase A.2: also check WarmStartPacket / MPI broadcast symbol
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  AA 1M WS-A.2 Full Run (MF+SPR) — W4 Gate — 16-node parity"
echo "║  run_id:       ${RUN_ID}"
...
echo "║  WS-A.1 np=16 ref: 169095645 MF=1146.174s SPR=1212.944s total=2440.301s"
echo "╚══════════════════════════════════════════════════════════════╝"
...
# ── Phase A.2 broadcast diagnostics ──────────────────────────────────
...
if os.path.isfile(diag_log):
    for line in open(diag_log, errors="replace"):
        # Phase 0.5 cross-model broadcast check (filterRatesMPI)
...
if mf_wall is not None:
    verify.append({"check": "mf_wall_W4", "status": "pass" if mf_wall <= MF_WALL_LIMIT else "fail",
                   "limit_s": MF_WALL_LIMIT, "actual_s": mf_wall,
                   "note": "§5.7 W4 gate: MF wall ≤ 900 s at np=16"})
...
record = {
    ...
    "summary": {
        ...
        "mf_wall_pass": mf_wall <= MF_WALL_LIMIT if mf_wall is not None else None,
    ...
    "branch":        "fca-lbfgs-ws",
    "non_canonical": True,
    "non_canonical_label": "FCA-WS Phase A.2 (cross-rank MPI broadcast) · ICX+MPI · AVX-512 · full MF+SPR · AA 1M · np=16",
    "group":         "fca_ws_a2",
}
...
vs_a1  = record["summary"]["vs_ws_a1_np16"]
vv     = record["summary"]["vs_vanilla_baseline"]
print(f"[16node] ─── FULL RUN SUMMARY (AA 1M WS-A.2 np=16 — W4 Gate) ───")
for v in verify:
    chk = v.get("check","?")
    ...
    if chk == "lnL":
        ...
        print(f"[16node]   ws_bcast (A2): {sts}  fields_max={ws_bcast_fields_max}  count={ws_bcast_count}")
    elif chk == "mf_wall_W4":
        print(f"[16node]   MF wall (W4):  {sts}  {v.get('actual_s')} s ≤ {v.get('limit_s')} s criterion")
if mf_wall:
    print(f"[16node]   MF:   {mf_wall:.3f} s  vs FCA-np16={FCA_MF}s ({vs_fca['mf_speedup']}×)  vs WS-A1-np16={WSA1_MF}s ({vs_a1['mf_speedup']}×)  vs vanilla={VANILLA_MF}s ({vv['mf_speedup']}×)")
if spr_wall:
    print(f"[16node]   SPR:  {spr_wall:.3f} s  vs FCA-np16={FCA_SPR}s ({vs_fca['spr_speedup']}×)  vs WS-A1-np16={WSA1_SPR}s ({vs_a1['spr_speedup']}×)  vs vanilla={VANILLA_SPR}s ({vv['spr_speedup']}×)")
if iqwall:
    print(f"[16node]   Total:{iqwall:.3f} s  vs FCA-np16={FCA_TOT}s ({vs_fca['total_speedup']}×)  vs WS-A1-np16={WSA1_TOT}s ({vs_a1['total_speedup']}×)  vs vanilla={VANILLA_TOT}s ({vv['total_speedup']}×)")
print(f"[16node]   Phase A.2 ws_bcast_fields: {'FIRED (' + str(ws_bcast_fields_max) + ' fields)' if ws_bcast_fields_max > 0 else 'NOT SEEN — FAIL'}")
print(f"[16node]   Overall W4: {'ALL PASS' if all_pass else 'FAIL'}")
PYEOF
