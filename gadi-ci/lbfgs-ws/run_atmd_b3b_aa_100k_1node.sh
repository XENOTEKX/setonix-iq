...
LABEL="AA_100k_atmd_b3b_np1_full_seed${SEED}"
...
# (a) K_outer > 1 activation — the primary goal of this test
if grep -q '\[ATMD Mode F\]' "${WORK_DIR}/iqtree_run.log" 2>/dev/null; then
    ...
    echo "  ✗ FAIL: [ATMD Mode F] line not found in log" >&2
    PASS=0
fi
