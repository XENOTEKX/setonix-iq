#!/bin/bash
# run_ws_a2_aa_1m_4node_full.sh — Full MF+SPR scaling run: warm-start A.2 binary, AA 1M, 4-node MPI.
#
# AA, 1M sites (100 taxa), 4 MPI ranks × 103 OpenMP threads = 412T total.
# 4 × Sapphire Rapids nodes (normalsr, 104 cores each).
#
# PURPOSE — Scaling measurement: Phase A.2 at np=4 where each rank owns ~56 models,
#   filterRatesMPI fires at model ~28, leaving ~28 remaining models to benefit from the
#   warm-start broadcast.  This is the np configuration expected to show the most significant
#   A.2 MF wall benefit.  Comparison against FCA Phase 0.5+0.6 np=4 baseline (job 168635615,
#   MF=1,974.476s SPR=3,982.142s total=5,956.618s).
#
# Gate: Scaling run (no named §5.7 gate for np=4 AA 1M).
#   Pass criteria:
#     - exit code = 0
...
#
# A/B refs:
#   FCA np=4 baseline  168635615 (MF=1974.476s  SPR=3982.142s  total=5956.618s)
...
# Related:   CHANGELOG (cg) WS-A.2 scaling, research/lbfgs-and-warmstart-implementation.md §12.8

#PBS -N ws-a2-aa-1m-4n
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=416
#PBS -l mem=2040GB
...
LABEL="AA_1m_ws_a2_np4_full_seed${SEED}"
...
if [[ "${#HOSTS[@]}" -ne 4 ]]; then
    echo "ERROR: expected 4 nodes, got ${#HOSTS[@]}" >&2; exit 9
fi
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  AA 1M WS-A.2 Full Run (MF+SPR) — Scaling np=4 — 4-node"
echo "║  run_id:       ${RUN_ID}"
...
echo "║  FCA np=4  ref: 168635615 MF=1974.476s SPR=3982.142s total=5956.618s"
...
echo ""
echo "[4node] Per-rank perf stat (IPC + LLC cache):"
...
echo ""
echo "[4node] gathering per-rank logs..."
...
echo "[4node] PROBE lines:        $(wc -l < "${WORK_DIR}/probe.log" 2>/dev/null || echo 0)"
echo "[4node] RANK-PROBE lines:   $(wc -l < "${WORK_DIR}/rank_probe.log" 2>/dev/null || echo 0)"
echo "[4node] MF-TIME lines:      $(wc -l < "${WORK_DIR}/mf_time.log" 2>/dev/null || echo 0)"
echo "[4node] MF-MPI-DIAG lines:  $(wc -l < "${WORK_DIR}/mf_diag.log" 2>/dev/null || echo 0)"
echo "[4node] Per-rank model counts (from MF-TIME):"
for r in 0 1 2 3; do
    n=$(grep -c "^MF-TIME: rank ${r} " "${WORK_DIR}/mf_time.log" 2>/dev/null || echo 0)
    echo "    rank ${r}: ${n} models evaluated"
done
echo "[4node] Per-rank family ownership (from MF-MPI-DIAG dispatch line):"
grep -E "MF-MPI-DIAG: rank [0-9]+/[0-9]+ owns " "${WORK_DIR}/mf_diag.log" 2>/dev/null | sed 's/^/    /' || true
echo "[4node] Phase A.2 broadcast diagnostic:"
...
TOL           = 1.0
# FCA np=4 baseline (168635615)
FCA_MF        = 1974.476
FCA_SPR       = 3982.142
FCA_TOT       = 5956.618
# FCA np=8 (for cross-scale reference)
FCA8_MF       = 1443.892
FCA8_SPR      = 2147.499
FCA8_TOT      = 3671.618
...
record = {
    ...
    "summary": {
        ...
        "vs_fca_np8_baseline": {
            "mf_speedup":    speedup(FCA8_MF,   mf_wall),
            "spr_speedup":   speedup(FCA8_SPR,  spr_wall),
            "total_speedup": speedup(FCA8_TOT,  iqwall),
        },
        "vs_vanilla_baseline": {
    ...
    "profile": {"nranks": nranks, "omp_per_rank": omp_per_rank,
                "placement": "mpi_4node_full_perrank_logs"},
    "build_tag":     "fca_ws_a2_icx_avx512",
    "branch":        "fca-lbfgs-ws",
    "non_canonical": True,
    "non_canonical_label": "FCA-WS Phase A.2 (cross-rank MPI broadcast) · ICX+MPI · AVX-512 · full MF+SPR · AA 1M · np=4",
    "group":         "fca_ws_a2",
}
...
vs_fca  = record["summary"]["vs_fca_np4_baseline"]
vs_fca8 = record["summary"]["vs_fca_np8_baseline"]
vv      = record["summary"]["vs_vanilla_baseline"]
print(f"[4node] ─── FULL RUN SUMMARY (AA 1M WS-A.2 np=4 — Scaling Run) ───")
for v in verify:
    chk = v.get("check","?")
    sts = "PASS" if v["status"] == "pass" else "FAIL"
    if chk == "lnL":
        print(f"[4node]   lnL:           {sts}  reported={v.get('reported')}  diff={v.get('diff')}")
    elif chk == "best_model":
        print(f"[4node]   model:         {sts}  ({v.get('reported')})")
    elif chk == "ws_bcast_fields_A2":
        print(f"[4node]   ws_bcast (A2): {sts}  fields_max={ws_bcast_fields_max}  count={ws_bcast_count}")
if mf_wall:
    print(f"[4node]   MF:   {mf_wall:.3f} s  vs FCA-np4={FCA_MF}s ({vs_fca['mf_speedup']}×)  vs FCA-np8={FCA8_MF}s ({vs_fca8['mf_speedup']}×)  vs vanilla={VANILLA_MF}s ({vv['mf_speedup']}×)")
if spr_wall:
    print(f"[4node]   SPR:  {spr_wall:.3f} s  vs FCA-np4={FCA_SPR}s ({vs_fca['spr_speedup']}×)  vs FCA-np8={FCA8_SPR}s ({vs_fca8['spr_speedup']}×)  vs vanilla={VANILLA_SPR}s ({vv['spr_speedup']}×)")
if iqwall:
    print(f"[4node]   Total:{iqwall:.3f} s  vs FCA-np4={FCA_TOT}s ({vs_fca['total_speedup']}×)  vs FCA-np8={FCA8_TOT}s ({vs_fca8['total_speedup']}×)  vs vanilla={VANILLA_TOT}s ({vv['total_speedup']}×)")
print(f"[4node]   Phase A.2 ws_bcast_fields: {'FIRED (' + str(ws_bcast_fields_max) + ' fields)' if ws_bcast_fields_max > 0 else 'NOT SEEN — FAIL'}")
print(f"[4node]   Overall: {'ALL PASS' if all_pass else 'FAIL'}")
PYEOF
