...
IQTREE="${IQTREE:-${ISO_DIR}/build-atmd-b3b/iqtree3-mpi-atmd-b3}"
...
LABEL="AA_1m_atmd_b3b_np4_full_seed${SEED}"
...
if [[ -f "${ISO_DIR}/build-atmd-b3b/.build-info.json" ]]; then
    EXPECTED_MD5=$(python3 -c "import json,sys; d=json.load(open('${ISO_DIR}/build-atmd-b3b/.build-info.json')); print(d.get('md5',''))" 2>/dev/null || echo "")
