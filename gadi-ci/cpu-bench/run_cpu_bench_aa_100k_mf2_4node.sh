#!/bin/bash
# run_cpu_bench_aa_100k_mf2_4node.sh — IQ-TREE3 MF2 CPU benchmark (MPI 4-node)
...
LABEL="AA_100k_mf2_np4_seed${SEED}"
...
if [[ "${#HOSTS[@]}" -ne 4 ]]; then
    ...
    echo "       Check PBS resource spec — #PBS -l ncpus=416 on normalsr." >&2
    exit 8
fi
HOST_A="${HOSTS[0]}"
HOST_B="${HOSTS[1]}"
