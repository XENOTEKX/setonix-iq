#!/bin/bash
# run_cpu_bench_dna_1m_spr_ws_a2_mtest_np2.sh
# Purpose: FCA + WS Phase A.2 — 2-node MPI, np=2, -m TEST, DNA 1M.
#
# Same binary as AA 100K headline row "C′" (job 170137866, md5 1547a906).
# WS A.2: rank 0 broadcasts converged rate params to all ranks after filterRatesMPI.
#
# Binary:  /scratch/rc29/as1708/iqtree3-mf-iso/build-mpi-iso/iqtree3-mpi-fca-ws-a2
#          md5 1547a906f1f75422514b0a0cdf2bc89e
#
# Parity:  lnL within ±1.0 of -59,208,019.212, model F81+F+G4
#          SPR ref: 168425675 (normalsr 103T, F81+F+G4, total=6114.5s)
#
# Purpose: Establish FCA+WS-A.2 np=2 baseline for DNA 1M.  Direct comparison
#   against script 3 (FCA no-WS np=2) to quantify warm-start benefit on a
#   large-but-tractable dataset where warm-start should matter.
#
# Compares to:
#   FCA no-WS np=1:  168913091 (MF=5121.2s)
#   FCA no-WS np=8:  168592214 (MF=1274.7s)
#   FCA no-WS np=2:  run_cpu_bench_dna_1m_spr_fca_nows_mtest_np2.sh (NEW)
#   AA 100K C' (WS np=2): 170137866 (MF=149.256s)
#
#PBS -N iq-dna-1m-ws-a2-np2
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=208
#PBS -l mem=1020GB
...
RUN_LABEL="${DATA_TYPE}_1m_ws_a2_mtest_np${NP}_seed${SEED}"
...
# Verify Phase A.2 WarmStartPacket symbol
...
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║  IQ-TREE FCA+WS-A.2 — DNA 1M, np=2, 2-node, -m TEST"
...
echo "║  node 0:   ${NODES[0]}"
echo "║  node 1:   ${NODES[1]}"
echo "║  FCA no-WS np=1 ref: 168913091 (MF=5121.2s)"
echo "║  FCA no-WS np=8 ref: 168592214 (MF=1274.7s)"
echo "╚═══════════════════════════════════════════════════════════════╝"
echo ""
...
record = {
  ...
  "headline_table_row": "FCA+WS-A.2 DNA-1M np=2",
  "compares_to": {
    "aa_100k_ws_a2_np2":    "170137866 (AA 100K FCA+WS-A.2 np=2, MF=149.256s)",
    ...
    "dna_1m_fca_np2_nows":  "run_cpu_bench_dna_1m_spr_fca_nows_mtest_np2.sh (NEW)",
  },
}
