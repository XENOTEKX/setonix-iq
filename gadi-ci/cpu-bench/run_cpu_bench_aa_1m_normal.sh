#!/bin/bash
# run_cpu_bench_aa_1m_normal.sh — IQ-TREE3 cpu_opt_merge CPU benchmark
# AA, 1M sites (100 taxa), Cascade Lake node, -nt 47, seed=1
...
#
# NOTE: 1M AA with -nt 47 is near the 192 GB normal-node memory cap.
#       Monitor /proc/meminfo during the run.
#
#PBS -N iq-cpu-aa-1m-clx
#PBS -P dx61
#PBS -q normal
#PBS -l ncpus=48
#PBS -l mem=190GB
...
ALIGNMENT="/scratch/dx61/sa0557/iqtree2/poc_builds/complex_data_shared/AA/LG+I+G4/taxa_100/len_1000000/tree_1/alignment_1000000.phy"
THREADS=47
SEED=1
DATA_TYPE="AA"
...
if [[ ! -f "${ALIGNMENT}" ]]; then
    echo "ERROR: alignment not accessible: ${ALIGNMENT}" >&2
    echo "  AA 1M tree_1 directory has drw-rwSrw- (no execute bit)." >&2
    echo "  Ask sa0557: chmod o+x .../len_1000000/tree_1" >&2
    exit 3
fi
...
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  CPU Benchmark — AA 1M · normal (CLX) · -nt ${THREADS}"
...
record = {
  ...
  "build_tag": "cpu_opt_merge_icx_avx512_clx",
