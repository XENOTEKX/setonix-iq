#!/bin/bash
# run_cpu_bench_aa_100k_spr_r1avx512_mtest.sh
# Purpose: R1+AVX-512 baseline — 1-node, non-MPI, -m TEST, AA 100K.
#
# Source: XENOTEKX/setonix-iq branch gadi-spr-r2-avx512 (commit 07966e4)
...
# NOT included: R2 (THP madvise MADV_HUGEPAGE on partial_lh)
#
# Pre-cloned to: /scratch/rc29/as1708/iqtree3-r1r2/src/iqtree3
# Build target:  /scratch/rc29/as1708/iqtree3-r1r2/build-icx-r1-avx512/iqtree3
#
# Parity: lnL within ±0.5 of -7,541,976.860, model LG+G4
#
#PBS -N iq-aa-100k-r1-avx512
#PBS -P dx61
#PBS -q normalsr
#PBS -l ncpus=104
#PBS -l mem=510GB
...
BUILD_DIR="${PROJECT_DIR}/build-icx-r1-avx512"
...
RUN_LABEL="${DATA_TYPE}_100k_r1avx512_seed${SEED}"
...
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║  IQ-TREE R1+AVX-512 — clone gadi-spr-r2-avx512 + build + bench"
...
echo "[r1avx512] icpx: $(icpx --version 2>&1 | head -1)"
...
echo "[r1avx512] commit: ${COMMIT}"
echo "[r1avx512] branch: $(git rev-parse --abbrev-ref HEAD)"
...
if grep -q 'MADV_HUGEPAGE' "${SRC_DIR}/tree/phylotreesse.cpp"; then
    echo "WARNING: R2 patch (MADV_HUGEPAGE) unexpectedly present — binary will include R2"
fi
echo "[r1avx512] patch check PASS — R1 present, R2 absent"
...
echo ""
echo "[r1avx512] building in ${BUILD_DIR}..."
rm -rf "${BUILD_DIR}"
mkdir -p "${BUILD_DIR}"
cd "${BUILD_DIR}"
...
echo "[r1avx512] binary: ${IQTREE}  md5: ${ACTUAL_MD5}"
...
echo "[r1avx512] library check: $(ldd "${IQTREE}" | grep -oE 'libiomp5|libmpi' | tr '\n' ' ')"
...
record = {
  ...
  "binary_note": "R1+AVX-512 (gadi-spr-r2-avx512 @ 07966e4): schedule(static)+NUMA first-touch+cmake P2+P3, no R2 THP, non-MPI, ICX 2025.3.2",
