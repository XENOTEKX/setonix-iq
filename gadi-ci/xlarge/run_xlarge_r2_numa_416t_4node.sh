#!/bin/bash
# run_xlarge_r2_numa_416t_4node.sh — IQ-TREE 3.1.2 (R2-patched, MPI build)
# on xlarge_mf.fa across **4 Gadi normalsr SPR nodes**, with
# **4 MPI ranks × 104 OpenMP threads each** = 416 total cores.
#
# Extends the ICX+R2 NUMA OMP single-node series:
#   icx_omp_pin_numa_ft_r2      1-node: 32T (1118s), 64T (690s), 104T (524s)
#   icx_mpi2x104_2node_*        2-node: 208T (334–342s, existing runs)
#   icx_mpi4x104_4node_*        4-node: 416T  ← this script
#
...
#   └─────────────────────┘  └─────────────────────┘
#
# Hypothesis — parallel to the 2-node result (342s ≈ 2× speedup over 104T):
#   (a) ~171 s  → roughly 4× speedup over 1×104 if IQ-TREE MPI scales linearly
#   (b) ~250 s  → diminishing returns from inter-node communication overhead
#   (c) ≥ 342 s → MPI traffic dominates; 4 nodes offer no improvement over 2
#
...
SEED="${SEED:-1}"
LABEL="${LABEL:-${DATASET_NAME}_${TOTAL_THREADS}t_icx_mpi${NRANKS}x${OMP_PER_RANK}_4node_fullnode_numa_ft_r2_v312}"
...
if [[ "${#HOSTS[@]}" -ne 4 ]]; then
    ...
    echo "       Check PBS resource spec — should be #PBS -l ncpus=416 on normalsr." >&2
    exit 9
fi
HOST_A="${HOSTS[0]}"
HOST_B="${HOSTS[1]}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  IQ-TREE 3.1.2 R2 — MPI 4-node full-node placement"
echo "║  run_id:        ${RUN_ID}"
echo "║  dataset:       ${DATA_PATH}"
echo "║  ranks × OMP:   ${NRANKS} × ${OMP_PER_RANK}  (= ${TOTAL_THREADS} total, 4 nodes)"
echo "║  node A:        ${HOST_A}  (rank 0, 104 cores)"
echo "║  node B:        ${HOST_B}  (rank 1, 104 cores)"
echo "║  node C:        ${HOST_C}  (rank 2, 104 cores)"
echo "║  node D:        ${HOST_D}  (rank 3, 104 cores)"
echo "║  binary:        ${IQTREE}"
echo "║  work_dir:      ${WORK_DIR}"
echo "╚══════════════════════════════════════════════════════════════╝"
echo "[4node-fullnode] hostfile:"
cat "${HOSTFILE}" | sed 's/^/    /'

# Verify 104 physical cores on head node before writing rankfile
...
echo "[4node-fullnode] head node topology: sockets=${LSCPU_SOCKETS} cores/socket=${LSCPU_COREPS} → physical_cores=${PHYSICAL_CORES}"
...
env = {
  ...
  "nranks": ${NRANKS}, "omp_per_rank": ${OMP_PER_RANK},
  "total_threads": ${TOTAL_THREADS},
  "placement": "mpi_4node_fullnode_numa_ft",
...
echo ""
echo "[4node-fullnode] Pass 1: ${NRANKS} ranks × ${OMP_PER_RANK} OMP across 4 nodes"
START_EPOCH=$(date +%s)
...
echo "[4node-fullnode] Pass 1 done: rc=${IQRC} wall=${WALL}s"

# ── Pass 2: perf stat ────────────────────────────────────────────────
if [[ "${IQRC}" -eq 0 ]] && command -v perf >/dev/null 2>&1; then
    echo "[4node-fullnode] Pass 2: perf stat (per-rank)"
    mpirun -np "${NRANKS}" \
        --hostfile "${HOSTFILE}" \
        --mca rmaps_base_mapping_policy "" \
        -rf "${RANKFILE}" \
...
# Collect perf from rank 0 only (head node)
agg = {}; perf_cmd = None
...
record = {
    ...
    "description": f"Gadi SPR R2 v3.1.2 — MPI 4-node full-node {nranks}×{omp_per_rank} = {threads}T",
    "timing": [{
    ...
    "build_tag":           "icx_mpi4x104_4node_fullnode_numa_ft_r2_v312",
    "non_canonical":       True,
    "non_canonical_label": "R2 · MPI 4-node",
}
out_path = os.path.join(runs, rid + ".json")
json.dump(record, open(out_path,"w"), indent=2, default=str)
print(f"[4node-fullnode] wrote {out_path}")
PYEOF

echo "[4node-fullnode] done."
exit "${IQRC}"
