...
**2026-05-27 design reset — ATMD-AID threshold tuning is retired.** P.7 v2 job 169343365 showed a structural Phase 0 bottleneck: 56 `+G4`/`+F+G4` models are individually expensive at AA 1M but below the static heavy threshold, so Phase 0 exceeded the 600s gate before Phase 1 could help. The active new direction is [event-driven-moldable-dispatch.md](event-driven-moldable-dispatch.md): model tasks are scheduled in short moldable epochs (`gs=1/2/4/8/np`), rate filters are explicit scheduler events, and costs are updated from MF-TIME telemetry between epochs.

**2026-05-27 implementation start — EDM v0 source + binary built (`4810a8ac`).** Active source `/scratch/rc29/as1708/iqtree3-mode-p-iso/src/iqtree3-mode-p-iso-p3` now has opt-in flags `--mf-edm`, `--no-mf-edm`, `--mf-edm-epoch-sec`, and `--mf-edm-group-size`. `CandidateModelSet::edmScheduleInitialEpochs()` schedules all ready model tasks through the existing Mode-P/AID communicator lattice instead of ATMD-AID's static light/heavy split: epoch 0 is a rate-sentinel tranche on the canonical reference family, then the remaining tasks are LPT-packed into a moldable tail epoch (default `gs=4`). `evaluateAll()` disables legacy MPGC/AID when EDM is active, suppresses the FCA loop by marking scheduled tasks with `MF_AID_HEAVY`, and reuses canonical checkpoint/warm-start broadcast plus `aidExecuteWaves()` for cohort execution with `EDM-DIAG`/`EDM-WAVE` markers. Rebuilt binary: `/scratch/rc29/as1708/iqtree3-mode-p-iso/build-mode-p-iso-p3/iqtree3-mpi-mode-p-iso-p3`, md5 `4810a8ac73e3b92b1f93b3f03ec04d57`. Validation: source diagnostics clean; `make -j4 iqtree3` passes with OpenMPI+ICX wrapper environment; binary contains `--mf-edm`, `EDM-DIAG`, `EDM-EPOCH`. A login-node two-rank smoke test reached alignment parsing then hit `SIGILL`, so runtime validation must be submitted on SPR compute nodes.
...
**2026-05-27 — ATMD-AID v2: empirical cost predictor + gs=2 LPT-collapse. Build 169341279 ✅ PASS. ISO-4 AID v2 job 169341954 ✅ PASS.** Root cause of P.7 v1 failure (job 169227541) confirmed: `LG+I+G4` (no +F) had `rate_mult=5 × freq_mult=1 = 5 < threshold 9` → classified LIGHT → ran in Phase 0 FCA for >420s. v2 changes: (1) **Empirical rate multipliers** — LG≈16.4s (ratio 1), LG+I≈62s (ratio 4), LG+G4≈176s (ratio 11), LG+I+G4≥420s (ratio 28). New: `+I+G4`→28 (was 5), `+G4`→11 (was 4), `+I`→4 (was 2). `+F` freq_mult: 3.0→1.2 (empirically +F adds ~20% not 300%). New avg=12.1, threshold=18.15: heavy set now `+I+G4` + `+F+I+G4` (56/224). `+I+R`/`+I*R` handled separately (8.0× per k vs plain `+R`/`*R` 4.0×). (2) **gs=2 LPT-collapse**: all gs=2 tail models into one wave with per-cohort LPT queue; `aid_cohort_id` member added; 4 waves total at np=16 (was 29 at np=4). Build 169341279 exit=0 8m33s md5=`0c493bd5`. ISO-4 v2 job 169341954 PASS: AID lnL=−7,541,976.862 (Δ=0.001), model=LG+G4, 2 waves, 141 AID-WAVE markers. `+I+G4` models confirmed in Phase 1 (HIVW+I+G4, BLOSUM62+I+G4, MTMAM+I+G4).
**2026-05-27 — P.7 v2 FAIL (job 169343365, cancelled at t≈14min). New root cause: Phase 0 +G4/+F+G4 bottleneck.** Rank 0 Phase 0 MF-TIME trace at AA 1M np=16: LG+F=16s, LG+F+I=62s, **LG+F+G4=209s**, PMB=16s, PMB+I=40s, **PMB+G4=174s**, MTART+F=16s, MTART+F+I=50s, **MTART+F+G4=277s** → Phase 0 internal time for rank 0 = 863s before Phase 1 even started (AID-DIAG confirmed "Phase 0 complete, entering Phase 1" at t=863s). Phase 1 just beginning (wave=0 gs=16, wave=1 gs=8 announced) when job cancelled. **Root cause**: `+G4` models have rate_mult=11 < threshold 18.15 → classified LIGHT → run in Phase 0 FCA. But at AA 1M, +G4 takes 174–277s per model (family-dependent: LG+F+G4=209s, PMB+G4=174s, MTART+F+G4=277s). There are 28 `+G4` + 28 `+F+G4` = 56 such expensive light models. Total light model serial time = 28×(16+62+175+16+62+209) = 15,120s → with LPT across 16 ranks ideal Phase 0 wall = **945s** (mathematical minimum), far exceeding the 600s gate. The bottleneck rank gets the LG reference family (287s) plus 2–3 additional +G4/+F+G4 models → ~800–900s Phase 0. **Structural limitation of ATMD-AID**: the Phase 0/Phase 1 split only delivers when "light" models are genuinely cheap (they are at AA 100K, ~10s each; not at AA 1M, 175–277s each). Making +G4/+F+G4 "heavy" instead does not help — it moves 56 more models into Phase 1, but the gs=2 LPT-collapsed wave then serially processes ~13 models per cohort × ~100s each = ~1300s per cohort, which is also fatal. **Architecture verdict**: ATMD-AID is correct for AA 100K (ISO-4 PASS) and for workloads with 2–8 truly dominant heavy models, but cannot pass P.7 (AA 1M np=16 ≤600s) in any threshold configuration. The light model tail at AA 1M is intrinsically expensive. **Next approach**: P.7-MPGC with `--mode-p --mode-p-group-size 4`. np=16 → 4 groups of 4 ranks; ALL models run through Mode P at gs=4 (no Phase 0/Phase 1 split); expected MF wall ≈ FCA_np=4 (1974s) / 4 = **~494s ≤ 600s** ✓. P.7-MPGC binary and run scripts already present; ISO-4/ISO-5 MPGC correctness verified (jobs 169197750, 169207131).
...
| **ISO-4 ATMD-AID v1** | ✅ **PASS** — job **169227435** completed with binary `3e79db19` | ✅ PASS (v1) | Final root cause was rank-local Phase 0 checkpoint/warm-start state entering collective AID Phase 1. Fix broadcasts canonical checkpoint and warm-start from rank 0 before waves. Wave 0 `MTVER+F+I+G` completed on all ranks with matching score `1.58894e+07` before wave 1, eliminating the earlier rank-local escape. AID: LG+G4, lnL=−7,541,976.854 (Δ=0.007), MF=345s, wall=360s, **29 waves**, 142 `AID-WAVE` starts. v1 cost predictor: `+I+G4` rate_mult=5, threshold missed → `+I+G4` misclassified as light. |
| **ISO-4 ATMD-AID v2** | ✅ **PASS** — job **169341954** completed with binary `0c493bd5` | ✅ PASS (v2) | Empirical cost predictor (rate_mult `+I+G4`: 5→28, freq_mult `+F`: 3.0→1.2) + gs=2 LPT-collapse (29→**2 waves**). BASE exit=0 wall=432s lnL=−7,541,976.853 (Δ=0.008) model=LG+G4 ✓. AID exit=0 wall=445s lnL=−7,541,976.862 (Δ=0.001) model=LG+G4 ✓. AID-WAVE markers: 141 ✓ (Phase 1 fired). Wave structure: wave=0 gs=4 1 model, wave=1 gs=2 55 models LPT-balanced across 2 cohorts per_cohort_max=28. `+I+G4` (no +F) models confirmed in Phase 1 waves (HIVW+I+G4, MTMAM+I+G4, BLOSUM62+I+G4 seen in wave=1). Wall 14m40s vs 30m limit. Build job 169341279 exit=0 8m33s. |
| **P.7 Perf gate — ATMD-AID v1** | AA 1M np=16 MF wall ≤ 600 s | ❌ **FAIL** — job **169227541** (binary `3e79db19`) | Phase 0 bottleneck: `LG+I+G4` misclassified light (rate_mult=5 < threshold 9), ran >420s in Phase 0 FCA, gate exceeded before Phase 1. |
| **P.7 Perf gate — ATMD-AID v2** | AA 1M np=16 MF wall ≤ 600 s | ❌ **FAIL** — job **169343365** (binary `0c493bd5`, cancelled at t≈14min) | New bottleneck: `+G4` and `+F+G4` models classified light (rate_mult=11/13.2 < threshold 18.15) but at AA 1M each takes 175–277s. 56 expensive light models → Phase 0 wall ~945s (LPT-ideal minimum). Rank 0 Phase 0 reached MF internal t=863s before Phase 1 started. Structural: no threshold choice fixes this for ATMD-AID at AA 1M. |
| **P.7 Perf gate — P.7-MPGC group_size=4** | AA 1M np=16 MF wall ≤ 600 s | ⏳ **NEXT** — switch to `--mode-p --mode-p-group-size 4` | No Phase 0/Phase 1 split; all 224 models run at gs=4 (4× speedup per model), 4 groups in parallel = same model throughput as FCA np=4. Expected MF wall ≈ FCA np=4 (1974s) / 4 = **~494s ≤ 600s** ✓. Correctness already verified at gs=2 (ISO-4 job 169197750, ISO-5 job 169207131). Increase `mode_p_group_size` to 4 in run script and resubmit. |
| **ISO-4 EDM** | AA 100K np=4 correctness gate `--mf-edm` | ✅ **PASS** — job **169350768** (binary `4810a8ac`) | All 8/8 checks pass. EDM lnL=−7,541,976.864 (Δ=0.003 vs ref) ✓. Best model LG+G4 ✓. EDM-DIAG=10, EDM-EPOCH=4, wave starts=818 ✓. Sentinel gs=4 (LG+F×4), tail gs=4 1 cohort 220 models. Wall 14m52s. |
| **P.7 EDM** | AA 1M np=16 MF wall ≤ 600 s `--mf-edm --mf-edm-group-size 4` | ❌ **FAIL** — job **169352556** (binary `4810a8ac`, PBS-killed ~30min wall) | Scheduler plan correct: sentinel gs=16, tail gs=4, 4 cohorts, 55 models/cohort. **Root cause 1:** Mode P gs=4 at AA 1M takes 150–241s/model (2–3× slower than FCA ~80s/model avg). The 4-way pattern split does not overcome allreduce communication overhead at 1M scale. **Root cause 2:** Tail contains `+I+G4`/`+R4`/`+I+R4` models (the expensive ones, 150–241s each) which the sentinel-boundary `filterRatesMPI` event would prune — but `explicit_filter_event_pending` means that filter is not yet wired. With filter: pruned tail ~50–70 models × ~40s avg / 4 cohorts ≈ 500–550s (near target). **Next step: implement explicit filter event at sentinel boundary before resubmitting P.7.** |
...
> **ATMD b4 / Mode P binary (active):** `iqtree3-mpi-mode-p-iso-p3` · md5 `0c493bd5ebaa06a72348ddafec3ae1a2` · built 2026-05-27 00:10 (ATMD-AID v2: empirical cost predictor + gs=2 LPT-collapse + `aid_cohort_id`, build job 169341279 exit=0 wall=8m33s) · prior builds: `3e79db19` (ATMD-AID v1 canonical state fix), `68031e28` (ATMD-AID v1 initial build), `cc3d403f` (ISO-5 auto-dispatcher), `63548e7c` (B.4-12/13/14/15 + P.6), `76cdfb19` (B.4-15 MPGC Inheritance), `ad77fa4e` (B.4-13+14), `43c313f3` (B.4-14 CSCH only), P.7-MPGC `efef6be4`, B.4-11 `85bf5c79`, B.4-9 `9660575a`
