...
| **G.4.0b — FreeRate (+R) gradient on the unscaled GPU path** | The +R rate + weight gradient (the exact L.0b.v/vi code that overflowed ~10⁵⁴ on CPU), recomputed unscaled in eigen-space. | **`|G-ratio|` < 0.01 for LG+R4** (the Mode-L FDCHECK gate that FAILED at 10⁵⁴) — i.e. the GPU unscaled path **does not overflow**. If it does, port the log-sum-exp-stable per-category reduction and re-gate. | **HIGHEST — this is the hypothesis test of the whole new direction.** |
| **G.4.1 — Standalone joint LM/L-BFGS driver** | Joint optimizer over (b, α, p_inv[, +R]) for one model, driven by K6/K7/K8; LM (BHHH curvature) with L-BFGS fallback; trust-region safeguard. | **(1) Converges to IQ-TREE's MLE: lnL rel ≤ 1e-9 for LG+G4 (−7541976.853) and a +R model.** **(2) GPU critical-path steps per converged model < the sequential baseline by a measured factor** (the *correct* GPU re-statement of the failed Mode-L L.1 traversal gate). **(3) per-model GPU wall < the 4.7×-slower stateless `-te` baseline.** | MEDIUM-HIGH (convergence robustness). |
