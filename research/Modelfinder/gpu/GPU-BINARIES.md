...
|---|---|---|---|---|
| **LIVE GPU (current)** | `build-gpu-on/iqtree3` | `96fa69ea` | JOLT + GPU HEAD, commit `9f3fee28` (G.8.2.0 profile-mixture EM weight-optimiser kill-switch). Kernels: free-Q, +F, +G, +R coverage, **+ `k1_node_mix` mixture lnL + `k2_derv_mix` mixture derivative + `gpuMixWeightEMCrossCheckOnce` EM-weight clean-room cross-checks**. | Any new run, `--jolt --gpu`, all diagnostics, coverage audits, current-feature benchmarks. **Default.** (md5 changes every rebuild — this is the 2026-06-19 G.8.2.0 build.) |
