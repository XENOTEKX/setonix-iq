...
|---|---|---|---|---|
| **LIVE GPU (current)** | `build-gpu-on/iqtree3` | `80ea98a4` | JOLT + GPU HEAD, commit `b855c3fe` (G.8.1b profile-mixture branch derivative). Kernels: free-Q, +F, +G, +R coverage, **+ `k1_node_mix` mixture lnL + `k2_derv_mix` mixture derivative clean-room cross-checks**. | Any new run, `--jolt --gpu`, all diagnostics, coverage audits, current-feature benchmarks. **Default.** (md5 changes every rebuild — this is the 2026-06-18 G.8.1b build.) |
