#!/bin/bash
# run_xlarge_mf2_full_2node.sh — MF2 binary, full IQ-TREE, 2-node MPI.
#
# Runs the MF2 dispatch binary (um09/iqtree3-mf2/build-mpi-mf2/iqtree3-mpi)
# with 2 MPI ranks × 104 OMP threads each (= 208 effective threads, 2 full
# SPR nodes) using the SAME protocol as all other full-IQ-TREE families:
#   - full IQ-TREE (no -m MF, no -te)
#   - free tree
#   - seed=1
#
# This is the direct 2-node extension of run_xlarge_mf2_full.sh (np=1).
# Results land on the MF2 Full IQ-TREE scaling curve at 208T.
#
# Placement: --hostfile + -rf rankfile (validated form from PBS 168000131).
#   rank 0 = node A  slot=0-103
#   rank 1 = node B  slot=0-103
#
# Walltime estimate: 104T np=1 takes ~494s.  With 2 ranks sharing
# ModelFinder + tree search via MPI, expect ~200-350s → 00:20:00 is safe.
#
#PBS -N iq-mf2-full-2node
#PBS -P um09
#PBS -q normalsr
#PBS -l ncpus=208
#PBS -l mem=400GB
...
if [[ "${#HOSTS[@]}" -ne 2 ]]; then
    ...
    echo "       Check PBS spec: #PBS -l ncpus=208 on normalsr." >&2
    exit 9
fi
HOST_A="${HOSTS[0]}"
HOST_B="${HOSTS[1]}"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  MF2 binary — full IQ-TREE — ${NRANKS} ranks × ${OMP_PER_RANK}T = ${TOTAL_THREADS}T (2 nodes)"
echo "║  run_id:      ${RUN_ID}"
...
echo "╚══════════════════════════════════════════════════════════════╝"
echo "[mf2-full-2node] hostfile:"; cat "${HOSTFILE}" | sed 's/^/    /'
echo "[mf2-full-2node] rankfile:"; cat "${RANKFILE}"  | sed 's/^/    /'
...
echo "[mf2-full-2node] rc=${IQRC} wall=${WALL}s"
...
record = {
  ...
  "description": (f"MF2 binary full IQ-TREE (free tree, seed=1) — "
                  f"np={nranks} 2-node full-node, {nranks}×{omp_per}T = {total_thr}T"),
  "timing": [{
...
print(f"[mf2-full-2node] wrote {out_path}")
PYEOF

echo "[mf2-full-2node] done."
exit "${IQRC}"
