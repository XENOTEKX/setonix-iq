#!/bin/bash
# run_xlarge_mf2_full_16node.sh — MF2 binary, full IQ-TREE, 16-node MPI.
#
# 16 MPI ranks × 104 OMP threads = 1664 effective threads, 16 full SPR nodes.
# Same protocol as all other full-IQ-TREE families:
#   - full IQ-TREE (no -m MF, no -te), free tree, seed=1
#
#PBS -N iq-mf2-full-16node
#PBS -P um09
#PBS -q normalsr
#PBS -l ncpus=1664
#PBS -l mem=3200GB
...
if [[ -s "${SHA256_LOCKFILE}" ]]; then
    expected="$(awk -v f="${DATA_BASENAME}" '/^[[:space:]]*#/ {print} $2==f {print $1}' "${SHA256_LOCKFILE}" | grep -v '^#' | head -1)"
    if [[ -n "${expected}" ]]; then
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  MF2 binary — full IQ-TREE — ${NRANKS} ranks × ${OMP_PER_RANK}T = ${TOTAL_THREADS}T (16 nodes)"
echo "║  run_id:  ${RUN_ID}"
...
echo "╚══════════════════════════════════════════════════════════════╝"
echo "[mf2-full-16node] hostfile:"; cat "${HOSTFILE}" | sed 's/^/    /'
echo "[mf2-full-16node] rankfile:"; cat "${RANKFILE}"  | sed 's/^/    /'
...
echo "[mf2-full-16node] rc=${IQRC} wall=${WALL}s"
...
record = {
  ...
  "description": (f"MF2 binary full IQ-TREE (free tree, seed=1) — "
                  f"np={nranks} 16-node full-node, {nranks}×{omp_per}T = {total_thr}T"),
  "timing": [{
...
print(f"[mf2-full-16node] wrote {out_path}")
PYEOF

echo "[mf2-full-16node] done."
exit "${IQRC}"
