#!/bin/bash
# run_xlarge_mf2_full_4node.sh — MF2 binary, full IQ-TREE, 4-node MPI.
#
# Runs the MF2 dispatch binary (um09/iqtree3-mf2/build-mpi-mf2/iqtree3-mpi)
# with 4 MPI ranks × 104 OMP threads each (= 416 effective threads, 4 full
# SPR nodes) using the SAME protocol as all other full-IQ-TREE families:
#   - full IQ-TREE (no -m MF, no -te)
#   - free tree
#   - seed=1
#
# This is the direct 4-node extension of run_xlarge_mf2_full.sh (np=1).
# Results land on the MF2 Full IQ-TREE scaling curve at 416T.
#
# Placement: --hostfile + -rf rankfile (validated form from PBS 168000131).
#   rank 0 = node A  slot=0-103
#   rank 1 = node B  slot=0-103
#   rank 2 = node C  slot=0-103
#   rank 3 = node D  slot=0-103
#
# Walltime estimate: 104T np=1 takes ~494s; 2-node ~325s.  4-node expected
# ~200-280s → 00:20:00 is safe.
#
#PBS -N iq-mf2-full-4node
#PBS -P um09
#PBS -q normalsr
#PBS -l ncpus=416
#PBS -l mem=800GB
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  MF2 binary — full IQ-TREE — ${NRANKS} ranks × ${OMP_PER_RANK}T = ${TOTAL_THREADS}T (4 nodes)"
echo "║  run_id:      ${RUN_ID}"
echo "║  dataset:     ${DATA_PATH}"
echo "║  binary:      ${IQTREE}"
echo "║  node A:      ${HOST_A}  (rank 0, slot=0-103)"
echo "║  node B:      ${HOST_B}  (rank 1, slot=0-103)"
echo "║  node C:      ${HOST_C}  (rank 2, slot=0-103)"
echo "║  node D:      ${HOST_D}  (rank 3, slot=0-103)"
echo "╚══════════════════════════════════════════════════════════════╝"
echo "[mf2-full-4node] hostfile:"; cat "${HOSTFILE}" | sed 's/^/    /'
echo "[mf2-full-4node] rankfile:"; cat "${RANKFILE}"  | sed 's/^/    /'
...
echo "[mf2-full-4node] rc=${IQRC} wall=${WALL}s"
...
host_a, host_b, host_c, host_d = "${HOST_A}", "${HOST_B}", "${HOST_C}", "${HOST_D}"
...
record = {
  ...
  "description": (f"MF2 binary full IQ-TREE (free tree, seed=1) — "
                  f"np={nranks} 4-node full-node, {nranks}×{omp_per}T = {total_thr}T"),
  "timing": [{
  ...
  "env": {
    ...
    "nodes": 4, "hosts": [host_a, host_b, host_c, host_d],
...
print(f"[mf2-full-4node] wrote {out_path}")
PYEOF

echo "[mf2-full-4node] done."
exit "${IQRC}"
